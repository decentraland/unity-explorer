...
CI is path-scoped: each project's workflows run on its own root + `unity-shared-dependencies/**` (`test.yml`/`build-unitycloud.yml` for Explorer, `avatar-preview-renderer-*.yml` for the renderer). Releases are independent: Explorer `v*` tags from `main` own the repo's *Latest*; the renderer auto-releases on merges to `dev` touching `avatar-preview-renderer/**` (tags `avatar-preview-renderer/v3.X.0`, minor auto-increments; manual dispatch stays available — see [`docs/build-and-ci.md` § Avatar Preview Renderer](docs/build-and-ci.md#avatar-preview-renderer)).
...
* **Introducing a null-returning contract without auditing every consumer.** When you change a converter or parse method to return `null` on malformed input (e.g. a `JsonConverter` that yields `null` instead of throwing), the nullability ripples to *every* consumer, and Newtonsoft will place `null` elements into a deserialized `List<T>`. Audit all call sites before flipping the contract, and fix the batch/collection iteration paths too (`foreach (T? item in list) { if (item == null) continue; ... }`), not just the single-value/retry paths. A `null` left in a batch dereferences into a `NullReferenceException` far from the converter (`LoadProfilesBatchSystem` had to skip null `Profile` entries once `ProfileConverter` started returning null for malformed avatars).
* **Applying local conventions to serialized JSON DTO fields.** Deserialized DTO fields follow the wire format: keep the JSON key casing (renaming breaks `JsonUtility` deserialization — suppress per file with `// ReSharper disable InconsistentNaming` above the namespace), initialize schema-*required* fields with `= null!` (`= default!` for generic parameters), declare schema-*optional* fields as `T?`, and include a schema-link comment on the class. The naming suppression applies only to deserialized DTO fields. `= null!` is allowed in exactly two places — schema-required DTO fields and test fixture fields assigned in `[SetUp]`/`[OneTimeSetUp]` — never in other production code. Full rules, edge cases, and reference example: [`docs/code-style-guidelines.md` § Serialized JSON DTOs](docs/code-style-guidelines.md#serialized-json-dtos-wire-format-exceptions).
...
* **Accessing `Option<T>.Value` without checking `Has` first.** When `Has` is false, `Value` is `default` (null for reference types) — a blind read silently propagates an invalid value far from its source. Branch on `option.Has` before touching `option.Value`. In production code do not call `Unwrap()` either — model absence explicitly (early return, `Option<T>` propagation) or use a factory that is valid by construction (e.g. `UserId.NewRandom()`). `Unwrap()` is acceptable only in test code for known-valid constants — there, never bare `.Value`.
