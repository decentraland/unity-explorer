...
* **Accessing `Option<T>.Value` without checking `Has` first.** When `Has` is false, `Value` is `default` (null for reference types) — a blind read silently propagates an invalid value far from its source. Branch on `option.Has` before touching `option.Value`, or call `option.Unwrap()` when validity is guaranteed by construction and a failure must be loud at the source. This applies to test code too — known-valid constants go through `Unwrap()`, never bare `.Value`.
