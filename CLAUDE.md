...
* **`null!` on DTO fields for schema-required properties.** When a DTO class deserializes a JSON response from an API whose OpenAPI/schema definition marks the field as `required`, initializing with `= null!` is the correct pattern (`= default!` when the field's type is a generic parameter) — it signals "non-null at runtime because the server guarantees it, but C# needs an initializer." The DTO class *must* include a schema-link comment (e.g. `// Server schema: social-service-ea docs/schemas.yaml#/…/SchemaName`) so reviewers can verify the contract. Fields that are *optional* in the schema must be declared nullable (`T?`) with no `null!`. A schema-optional field may be kept non-nullable only when every entity type the client consumes guarantees it in practice (e.g. `metadata` on catalyst entities) — state that strengthened contract in the schema-link comment. A field absent from the linked schema is optional unless verified against its actual source. This exception applies only to deserialized DTO fields — local variables, method return types, and non-DTO code must never use `null!`; use `T?` and propagate the nullable contract instead.
* **Renaming wire-format DTO fields to satisfy naming rules.** Serialized JSON DTO fields bind to the wire format by name, so the PascalCase members rule does not apply to them — renaming breaks deserialization. `[JsonProperty]` only rescues the Newtonsoft path; DTOs also parsed with Unity's `JsonUtility` (e.g. `SceneEntityDefinition` via `WRJsonParser.Unity`) have no rename mechanism at all. Keep the wire-format casing and suppress the inspection per file with `// ReSharper disable InconsistentNaming` above the namespace declaration. Like the `null!` exception, this applies only to serialized DTO fields — never suppress naming warnings on regular code.
