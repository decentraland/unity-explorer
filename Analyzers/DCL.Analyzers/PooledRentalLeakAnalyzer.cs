...
namespace DCL.Analyzers
{
    ...
    public sealed class PooledRentalLeakAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static void AnalyzeBlock(CodeBlockAnalysisContext context)
        {
            ...
            // a local is disqualified (silent) on ANY use that is not a provably-local read:
            // Release(local)/Return(local), any argument pass, return, store, alias, capture, using
