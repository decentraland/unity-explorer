...
namespace DCL.Analyzers.Tests
{
    ...
    {
        // Minimal UnityEngine.Pool surface: the analyzer matches on metadata names
        // (ListPool`1, ObjectPool`1, IObjectPool`1, ...), so stubs are enough to
        // exercise every path, including the IObjectPool interface-implementation match.
...
" + members + @"
        ...
        // PlayerTransformPropagationSystem shape: the rental is assigned inside another
        // call's argument list, so the value escapes through the argument even though the
        // local itself is only member-written afterwards - must stay silent.
        [Test]
