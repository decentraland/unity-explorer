...
namespace DCL.Analyzers.Tests
{
    ...
    {
        // Minimal base-system surface: the analyzer anchors on the
        // ECS.Abstract.BaseUnityLoopSystem metadata name (with a simple-name
        // fallback for relocated/stub bases), so a stub is enough.
...
" + updateBody + @"
        ...
        // [HotPath] opts any method into allocation checking - no ECS system required
        // (mikhail-dcl's hot-path policy: URL handlers, MCP runtime).
        [Test]
...
public class UrlResolver
{
    ...
    public void Resolve()
    {
        ...
        // CreateGltfAssetFromRawGltfSystem shape: exception objects constructed on failure
        // branches (wrapped into results, not thrown) are error-path work, never per-frame
        // pressure - must stay silent.
        [Test]
...
public class Widget : NotASystem
{
    ...
    protected override void Update(float t)
    {
        ...
        {
            // no BaseUnityLoopSystem anywhere: the compilation-start guard bails out
            var test = new CSharpAnalyzerTest<AllocationInSystemUpdateAnalyzer, DefaultVerifier>
            {
                TestCode = @"
public abstract class NotASystem
...
");
        ...
        {
            // Update bodies in this repo are one-line dispatchers; the per-frame work
            // (and the allocations) live in the [Query] methods the generated code calls
            var test = new CSharpAnalyzerTest<AllocationInSystemUpdateAnalyzer, DefaultVerifier>
            {
                TestCode = @"
namespace ECS.Abstract
...
public partial class MoveSystem : ECS.Abstract.BaseUnityLoopSystem
{
    ...
    private void HandleMove(float t)
    {
        ...
        {
            // ECS.Abstract.BaseUnityLoopSystem resolves in this compilation, so the
            // simple-name fallback must not accept an unrelated type sharing the name
            var test = new CSharpAnalyzerTest<AllocationInSystemUpdateAnalyzer, DefaultVerifier>
            {
                TestCode = @"
namespace ECS.Abstract
...
");
        ...
        {
            // base class lives outside ECS.Abstract: only the simple-name fallback matches
            var test = new CSharpAnalyzerTest<AllocationInSystemUpdateAnalyzer, DefaultVerifier>
            {
                TestCode = @"
namespace Stubs
