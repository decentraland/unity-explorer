float4 fragDoubleShadeFeather(VertexOutput i, half facing : VFACE) : SV_TARGET 
{
    ...
    // 1st ShadeMap
    int n1st_ShadeMapArrID = -1;//_1st_ShadeMapArr_ID;
    float4 _1st_ShadeMap_var = float4(Set_BaseColor.rgb, _MainTex_var.a);//lerp(SAMPLE_1ST_SHADEMAP(TRANSFORM_TEX(Set_UV0, _1st_ShadeMap),n1st_ShadeMapArrID),float4(Set_BaseColor.rgb, _MainTex_var.a),_Use_BaseAs1st);
    ...
    // 2nd ShadeMap
    int n2nd_ShadeMapArrID = -1;//_2nd_ShadeMapArr_ID;
    float4 _2nd_ShadeMap_var = _1st_ShadeMap_var;//lerp(SAMPLE_2ND_SHADEMAP(TRANSFORM_TEX(Set_UV0, _2nd_ShadeMap),n2nd_ShadeMapArrID),_1st_ShadeMap_var,_Use_1stAs2nd);
    ...
    int nSet_1st_ShadePositionArrID = -1;//_Set_1st_ShadePositionArr_ID;
    float4 _Set_1st_ShadePosition_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_SET_1ST_SHADEPOSITION(TRANSFORM_TEX(Set_UV0,_Set_1st_ShadePosition),nSet_1st_ShadePositionArrID);

    int nSet_2nd_ShadePositionArrID = -1;//_Set_2nd_ShadePositionArr_ID;
    float4 _Set_2nd_ShadePosition_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_SET_2ND_SHADEPOSITION(TRANSFORM_TEX(Set_UV0,_Set_2nd_ShadePosition),nSet_2nd_ShadePositionArrID);
    // SHARED END
    ...
    // SHARED START
    int nSet_HighColorMaskArrID = -1;//_Set_HighColorMaskArr_ID;
    float4 _Set_HighColorMask_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_HIGHCOLORMASK(TRANSFORM_TEX(Set_UV0, _Set_HighColorMask), nSet_HighColorMaskArrID);
    ...
    int nHighColor_TexArrID = -1;//_HighColor_TexArr_ID;
    float4 _HighColor_Tex_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_HIGHCOLOR(TRANSFORM_TEX(Set_UV0, _HighColor_Tex), nHighColor_TexArrID);
    ...
    int nSet_RimLightMaskArrID = -1;//_Set_RimLightMaskArr_ID;
    float4 _Set_RimLightMask_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_SET_RIMLIGHTMASK(TRANSFORM_TEX(Set_UV0, _Set_RimLightMask), nSet_RimLightMaskArrID);
    ...
    //Composition: HighColor and RimLight as _RimLight_var (studio: _RimLightIntensity scales the rim term)
    float3 _RimLight_var = lerp( Set_HighColor, (Set_HighColor+Set_RimLight*_RimLightIntensity), _RimLight );
    // Rimlight - End
    ...
    float fRotate_MatCapUV = 0.0f;
    if(_sign_Mirror < 0)
    {
        _Right_Axis = -1 * _Right_Axis;
        fRotate_MatCapUV = -1 * _Rotate_MatCapUV;
    }
    else
    {
    ...
    if (_IsStylizedMetallic > 0 && _MatCap_SamplerArr_ID >= 0)
    {
        ...
        // Strength of the metallic effect (0 = none, 1 = full): studio-promoted material property.
    ...
    // Studio unlock: the UTS additional-lights loop below is re-enabled (commented out in the
    // package shader pending a Forward+ rework). The studio pipeline runs classic Forward with
    // per-pixel additional lights, which is the UTS_LIGHT_LOOP path.
        ...
        int pixelLightCount = GetAdditionalLightsCount();
            ...
            for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); lightIndex++)
            {
                ...
                // if (iLight != i.mainLightID)
                {
                    ...
                    //v.2.0.5
                    int n1st_ShadeMapArrID = -1;//_1st_ShadeMapArr_ID; (stubbed like the main-light path)
                    float4 _1st_ShadeMap_var = float4(Set_BaseColor.rgb, _MainTex_var.a);//lerp(SAMPLE_1ST_SHADEMAP(...), ..., _Use_BaseAs1st); (no shade maps in this variant)
                    ...
                    //v.2.0.5
                    int n2nd_ShadeMapArrID = -1;//_2nd_ShadeMapArr_ID;
                    float4 _2nd_ShadeMap_var = _1st_ShadeMap_var;//lerp(SAMPLE_2ND_SHADEMAP(...), _1st_ShadeMap_var, _Use_1stAs2nd);
                    ...
                    int nSet_2nd_ShadePositionArrID = -1;//_Set_2nd_ShadePositionArr_ID;
                    float4 _Set_2nd_ShadePosition_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_SET_2ND_SHADEPOSITION(...);
                    
                    int nSet_1st_ShadePositionArrID = -1;//_Set_1st_ShadePositionArr_ID;
                    float4 _Set_1st_ShadePosition_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_SET_1ST_SHADEPOSITION(...);
                    ...
                    int nSet_HighColorMaskArrID = -1;//_Set_HighColorMaskArr_ID;
                    float4 _Set_HighColorMask_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_HIGHCOLORMASK(...);
                    float _Specular_var = 0.5*dot(halfDirection, lerp(i.normalDir, normalDirection, _Is_NormalMapToHighColor)) + 0.5; //  Specular                
                    float _TweakHighColorMask_var = (saturate((_Set_HighColorMask_var.g + _Tweak_HighColorMaskLevel))*lerp((1.0 - step(_Specular_var, (1.0 - pow(abs(_HighColor_Power), 5)))), pow(abs(_Specular_var), exp2(lerp(11, 1, _HighColor_Power))), _Is_SpecularToHighColor));
    
                    int nHighColor_TeArrID = -1;//_HighColor_TexArr_ID;
                    float4 _HighColor_Tex_var = float4(1.0f, 1.0f, 1.0f, 1.0f);//SAMPLE_HIGHCOLOR(...);
                    float3 _HighColor_var = (lerp((_HighColor_Tex_var.rgb*_HighColor.rgb), ((_HighColor_Tex_var.rgb*_HighColor.rgb)*Set_LightColor), _Is_LightColor_HighColor)*_TweakHighColorMask_var);
