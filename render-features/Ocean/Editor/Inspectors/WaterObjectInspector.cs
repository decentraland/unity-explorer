...
{
    [CustomEditor(typeof(OceanObject))]
    [CanEditMultipleObjects]
    public class WaterObjectInspector : Editor
    {
        private OceanObject component;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                OceanObject water = (OceanObject)currentTarget;
                water.FetchWaterMaterial();
            }

            if (OceanObject.Instances.Count > 1)
            {
                EditorGUILayout.Space();

                showInstances = EditorGUILayout.BeginFoldoutHeaderGroup(showInstances, $"Instances ({OceanObject.Instances.Count})");
                ...
                if (showInstances)
                {
                    ...
                    {
                        foreach (OceanObject obj in OceanObject.Instances)
                        {
