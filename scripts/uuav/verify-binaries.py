...
Six independent checks, each of which fails the run on its own:
  ...
  artifacts       sha256 of every shipped .dylib/.dll matches the lock
  ...
  rust-source     the digest of native/src matches what this target's cdylib
                  was built from - the crate has no upstream revision to pin,
                  so the source digest is the pin
  ffmpeg-builder  the FFmpeg build script is unchanged *in substance*, and the
                  ...
                  15-minute rebuild on two platforms
...
expected to differ (see docs/uuav.md) but the configure line
is not.
...
def tree_sha256(root: str, suffix, prune=()) -> tuple[str, int]:
    ...
    edit to native/src that was never followed by a rebuild visible: the
    shipped cdylib's own hash still matches the lock in that case, because
    nobody rebuilt it.

    *suffix* is one extension or a list of them. Directory names in *prune* are
    skipped wherever they occur - `tests`, `benches` and the build directories,
    none of which end up inside a shipped binary, so including them would turn
    the lock red on edits that cannot change what ships.
    """
...
def cargo_lock_closure(path: str, roots) -> tuple[str, int]:
    ...
    The workspace now has six members, only some of which produce a shipped
    artifact. Digesting the whole lock file makes a dependency added to a
    member that ships nothing look identical to one added to the core, and the
    only remedy the lock offers is "rebuild both platforms and relock" - a
    macOS and a Windows build of a byte-identical binary. Walking the closure
    of the shipping roots instead means the digest moves exactly when the set
    of crates compiled into a shipped binary moves.
...
def probe_toolchain_component(component: str, native_dir: str) -> str | None:
    ...
    written here is the value Gate B will later compare against. Probed from
    native_dir so rustup's rust-toolchain.toml resolution applies, exactly as
    it does for the build itself.
    """
    if component in ("rustc", "cargo", "clang"):
        return _first_line([component, "--version"], native_dir)
    if component in ("cl", "link"):
        return _first_line([component], native_dir)
...
def resolve_toolchain_pin(target: str, spec: dict, toolchain_file: str | None,
    ...
    keeping the old toolchain identity: a relock on a different rustc / MSVC /
...
def check_rust_source(repo: str, lock: dict, target: str, spec: dict,
    ...
    """Tie this target's cdylib to the native/src state that produced it.
    ...
    shipping cdylibs from different source states.
    """
    ...
    if actual != spec["rust"]["source_digest"]:
        return [f"[{target}] {source['path']} changed but this target's cdylib was "
                f"not rebuilt and relocked\n"
                f"    expected digest {spec['rust']['source_digest']} "
                f"({spec['rust']['source_files']} files)\n"
                f"    actual   digest {actual} ({files} files)\n"
                f"    The shipped cdylib no longer corresponds to native/src."]
    return []
...
def strip_shell_comments(line: str) -> str:
    ...
    as one: `sed -n 's/^#define ...'` in the macOS builder carries a literal
    ...
    comment, so it expands to nothing - is how both builders comment *inside* a
...
def normalize_build_script(path: str) -> str:
    ...
    lines removed. Same move as crates/uuav-adapter/build.rs, which digests the
    core's FrameInfo *declaration* with comments stripped and whitespace
    collapsed so that a comment edit in the frozen core is not a false positive.
    ...
    Unlike rust_source, which stays a text-exact digest: the release profile
    carries debug info, so a comment in a .rs file shifts line numbers and
    genuinely changes the compiled bytes. Nothing a shell comment can do
    reaches the compiled FFmpeg.
    """
    lines = []
    with open(path, encoding="utf-8") as handle:
        for raw in handle:
...
def configure_from_script(path: str) -> list[str]:
    ...
    macOS build factors the invocation into a build_arch() function plus a
    SANDBOX_FLAGS array applied to every slice, so both the flags on the
    `configure` line and any *FLAGS=( ... ) array feeding it are collected into
    one sorted set. Per-slice and toolchain-specific flags (--prefix, --arch,
    --cc) and the configure probe's --help are dropped: they vary by slice and
    are normalized out of the line read back from a built library too. A stable
    superset is enough here - any edit to the script is caught independently by
    its builder_sha256.
    """
    with open(path, encoding="utf-8") as handle:
        text = handle.read()
...
def main() -> int:
    ...
    for target, spec in lock["targets"].items():
        note(f"{target}: {len(spec['artifacts'])} artifacts, FFmpeg "
             ...
             f"cdylib from native/src {spec['rust']['source_digest'][:12]}")
