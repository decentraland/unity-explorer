...
macos-universal)
    PLATFORM_DIR="macOS"
    CARGO_ARTIFACTS=(libuuav.dylib uuav-adapter)
    ;;
windows-x86_64)
    PLATFORM_DIR="x86_64"
    CARGO_ARTIFACTS=(uuav.dll uuav-adapter.exe)
    ;;
*)
...
if [[ ! -d "$FIRST_TREE/$FFMPEG_REL" ]]; then
    echo "error: $FFMPEG_REL is missing; build FFmpeg before the gate - it is built once and both cargo builds reuse it" >&2
    exit 1
fi
...
for name in "${CARGO_ARTIFACTS[@]}"; do
    ...
    echo "::error::[$TARGET] $name is not reproducible on this runner. Two builds of $REV, same machine, same canonical path, source trees named '$(basename "$FIRST_TREE")' and '$(basename "$SECOND_TREE")', produced different bytes: $differing byte(s) differ ($first_bytes vs $second_bytes bytes; $where). Something in the build is reading the clock, the source path or an unordered container - see the /Brepro and canonical-path notes in Explorer/Assets/Plugins/UUAV/native/.cargo/config.toml and scripts/uuav/build-canonical.sh."
