...
macos-universal)
    PLATFORM_DIR="Mac"
    CARGO_ARTIFACTS=(segment-server.dylib)
    ;;
windows-x86_64)
...
RUNTIME_REL="$PLUGIN/SegmentServerWrap/Libraries/$PLATFORM_DIR"

FIRST_TREE="$(git rev-parse --show-toplevel)"
REV="$(git -C "$FIRST_TREE" rev-parse HEAD)"

WORK="${RUST_SEGMENT_REPRO_WORK:-$(dirname "$FIRST_TREE")}"
SECOND_TREE="$WORK/rust-segment-repro-second-tree"
KEEP="$WORK/rust-segment-repro-first-build"
...
git -C "$SECOND_TREE" sparse-checkout set "$PLUGIN" scripts/rust-segment
...
build_in() {
    ...
    (cd "$tree" && bash scripts/rust-segment/build-canonical.sh)
    endgroup
}
...
for name in "${CARGO_ARTIFACTS[@]}"; do
    ...
    echo "::error::[$TARGET] $name is not reproducible on this runner. Two builds of $REV, same machine, same canonical path, source trees named '$(basename "$FIRST_TREE")' and '$(basename "$SECOND_TREE")', produced different bytes: $differing byte(s) differ ($first_bytes vs $second_bytes bytes; $where). Something in the build is reading the clock, the source path or an unordered container - see scripts/rust-segment/build-canonical.sh for how the canonical path removes the source-path input, and .native/.cargo/config.toml for the linker flags that remove the clock."
    printf '  DIFFERS    %-22s %s bytes differ\n' "$name" "$differing"
