...
# --- 1. working-tree mode: every fixture is untracked, all lines count as added
...
# --- 2. clean tree must be a silent pass
git -C "$work" add -A
commit -m fixtures
out="$(run_linter --working-tree)"; rc=$?
[ "$rc" -eq 0 ] && [ -z "$out" ] || fail "clean tree rc=$rc out='$out', want silent 0"

# --- 3. diff mode: only lines added since the base commit may report
