...
- **`avatar-preview-render-server.yml`** — builds the native Linux render server ([renderer README § Render server](../avatar-preview-renderer/README.md#render-server-native-cpu)) from the `t_linux_render_server` Unity Cloud template target (Linux x86_64, scripting define `DCL_RENDER_SERVER`; IL2CPP and the graphics APIs come from ProjectSettings), packages the player with `avatar-preview-renderer/RenderServer/` as `avatar-preview-render-server-v<version>-linux-x86_64.tar.gz`, and smoke-tests the image natively on the runner (one base wearable, checked for two 1024×1024 RGBA stills). The release workflow calls it and attaches the package to the same release as the WebGL zip. A failed render server build does **not** hold the release back: the release ships without the package and the run warns. Dispatch it by hand to try a branch; the package is then a run artifact.
