...
- **A targetless aim has no wrong answer to give: it reports the entity the ray reached.** `IsExpectedTarget` only rejects a hit when the intent named a `TargetEntityId` (or is a sweep release bound to its press entity), so an aim point or screen point on its own accepts whatever qualified entity the reticle found first — including one nearer than the aim point, whose handler then fires. `BlockedBy*` is reported for a nearer collider only in `DiagnoseUnqualified`'s aim-point-only path, i.e. when that collider has **no** `PointerEvents`; a clickable occluder is a hit, not a block (observed: an aim 1 m past a toggle clicked the toggle and returned its `crdtEntityId`). The result always carries `CrdtEntityId`, which is the only channel a driver can compare against what it aimed at — and naming `TargetEntityId` alongside the aim converts the same situation into a line-of-sight block.
...
- **A virtual-device drag across the world drags nothing there, and its `Ok` is not evidence it did.** `UiGestureResult` carries no target: a device gesture reports that the states were replayed. Two things can happen over the world. If the held button engages the camera pan — `Camera.TemporalLock` is the left mouse button, so the cursor turns to `Panning` and `PlayerOriginatedRaycastSystem` resets its raycast — `UiVirtualDeviceGestureSystem`'s per-frame cursor re-check fails the gesture with "the drag panned the camera instead of dragging" rather than reporting a delivery it did not make. If it does not engage, the gesture completes with a bare `Ok`: `CursorState.Panning` additionally requires `CursorComponent.PositionIsDirty` (set by `UpdateCameraInputSystem` only while the Camera action map is enabled) plus a free cursor, a pointer off UI, an in-bounds position and a non-SDK camera, and two measured drags over a scene's stroke canvas (20 and 45 frames, cursor `Free`) neither panned nor painted. A locked cursor fails the gesture up front. Sweeping a held pointer across the world is `sweep_pointer` (press, turn the camera, release) — which is also how a human does it.
