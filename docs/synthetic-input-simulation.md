...
| Pointer aim + buttons on entities | `SyntheticPointerInput` (in `DCL.Interaction`, the pipeline's own contract surface): a single-frame post of an aim point and/or button edge. A **screen-point** aim is first refused if UI covers that pixel (see the divergence below); a **world** aim is not. `PlayerOriginatedRaycastSystem` builds the reticle ray through the aim and echoes what it consumed; `ProcessPointerEventsSystem` applies the button edges under the same qualification gates as real input and clears the post. A post made for a gesture that **named an entity** also carries that entity (`TargetWorld` + `TargetEntity`, a nullable pair — `Entity.Null` is *not* `default(Entity)`, and a defaulted target once restricted every post to an entity that cannot exist, which withheld the edge from all entities and dropped the root broadcast with it), and the button edge is then delivered to it alone: the ray decides what is under the reticle, so without that restriction the edge lands on whatever it found — a nearer interactable, or a proximity entity the aim never pointed at — and fires that entity's handler while the driver is told its aim was blocked. |
...
| Global SDK input actions | `PrepareGlobalInputEventsSystem` appends the synthetic button edges to the per-frame global-events buffer (deduped against real same-frame presses). `ProcessPointerEventsSystem` then removes from that buffer every edge it landed entity-bound on a hovered, qualified entity, per action, the frame it fires. An edge that named a target entity is never appended in the first place: it is not a broadcast, so an aim the pipeline could not deliver reaches nobody instead of leaving the scene root observing a press the driver was told missed. |
...
**Virtual devices (fidelity).** `AutomationVirtualDevices` registers a `DclAutomationMouse` + `DclAutomationKeyboard` for the session. Layout-path bindings resolve them into **both** input-action graphs — the serialized asset driving the UI input module and the `DCLInput.Instance` clone gameplay polls — which is the point: an injected state event behaves like a real device for every consumer. `UiVirtualDeviceGestureSystem` replays move/click/drag/key gestures one state per frame (the drag goes through `DragWithDevicesAsync`, which owns its timeout and pairs the result with the UI cover at both ends as a `UiDeviceDragOutcome`); while a pointer gesture runs, the frame-stamped `SyntheticCursorState` suppresses the cursor system's OS-cursor warps so nothing fights the injected positions. Pointer gestures require a free cursor (locked/panning fails the gesture rather than mutating the lock).
...
- **A targetless aim has no wrong answer to give: it reports the entity the ray reached.** `IsExpectedTarget` only rejects a hit when the intent named a `TargetEntityId` (or is a sweep release bound to its press entity), so an aim point or screen point on its own accepts whatever qualified entity the reticle found first — including one nearer than the aim point, whose handler then fires. That is the contract of `click_at` and of an `x/y/z`-only `click_entity`: the pixel or the point is the address, and the result's `CrdtEntityId` is what a driver compares against its intention. **An intent that names an entity is different: the posted edge carries that entity and the pipeline withholds it from anything else**, so a blocked or unqualified aim reports `BlockedBy*` having delivered no button — to the blocker or to the scene root. A refusal on an aim that named no entity (a screen point, an `x/y/z` point) still broadcasts to the root, which is what an unqualified edge does for real input too; and because `ComposeGestureAsync` stops after a press that did not hit, that broadcast is a `PetDown` with no `PetUp` — a scene holding root-level pointer state keeps holding it until its own timeout. Before that restriction existed the report was a post-hoc diagnosis of an edge already delivered: a `hit:false` fired the blocker's handler (two refused clicks toggled a scene panel twice), and it also left the press in the scene-root broadcast buffer. What a refusal still delivers is `PetHoverEnter`/`PetHoverLeave` on whatever the ray reached — hover follows the ray for real input too, so an occluder a human's cursor passes over sees exactly the same pair.
...
- **A virtual-device drag across the world reaches no UI, and says so.** `UiGestureResult` carries no target — a device gesture reports that the states were replayed — so `UiAutomationServices.DragWithDevicesAsync` reads `TryFindUiCoverAt` at both end pixels *before* installing the gesture (the drag moves the pointer itself) and returns a `UiDeviceDragOutcome` carrying both covers, with null meaning the world. Over the world at both ends the outcome also carries a `DeliveryNote` saying no UI element received the drag, because a bare `Ok` there reads as a delivery: front-ends surface it as `pointerOver` + `info` (`ui_drag`) and `pointerOverStart`/`pointerOverEnd` + `info` (the AltTester drag probe). The other outcome is the camera pan: `Camera.TemporalLock` is the left mouse button, so a held button over the world can turn the cursor to `Panning` — `PlayerOriginatedRaycastSystem` resets its raycast while panning — and `UiVirtualDeviceGestureSystem`'s per-frame cursor re-check then fails the gesture with "the drag panned the camera instead of dragging". Whether it engages is not the driver's to control: `CursorState.Panning` also requires `CursorComponent.PositionIsDirty` (set by `UpdateCameraInputSystem` only while the Camera action map is enabled), a free cursor, a pointer off UI, an in-bounds position and a non-SDK camera — two measured drags over a scene's stroke canvas (20 and 45 frames, cursor `Free`) neither panned nor painted, which is exactly the case the note exists for. A locked cursor fails the gesture up front. Sweeping a held pointer across the world is `sweep_pointer` (press, turn the camera, release) — which is also how a human does it.
