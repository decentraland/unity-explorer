...
| Held pointer + camera sweep | No seam of its own: `SyntheticInputAgent.SweepAsync` composes the two above — a press leg, a camera-look hold while it is held, then a release leg. This is the only way to sweep the ray a scene samples: `PrimaryPointerInfo.WorldRayDirection` is built from the camera, so turning the camera moves it, while dragging the virtual mouse across the world pans the camera instead (see the divergences). |
...
- **The injected pointer steers the cursor only while a gesture is running.** The position is frame-stamped; once the gesture stops re-asserting it the cursor is the OS mouse's again, so the reticle returns to wherever the real cursor sits. Anything that must observe the injected pointer has to observe it during the gesture.
- **A sweep does not pin the reticle to the pressed target.** No aim is posted between the press and the release, which is what lets the sampled ray follow the camera; the release is aimed again at the original target, so it lands even after the camera turned (reported with `upRayMissed` if the pipeline disagreed).
