...
The `archipelago-heartbeats` flag is a launch-snapshotted publisher kill switch. An absent or `true` value preserves the current one-heartbeat-per-second behavior; `false` skips packet construction and socket sending while connection maintenance continues. A backend flag change affects only sessions launched after that change, so rollout must allow existing sessions to cycle before heartbeat intake is retired.

Pulse and the Archipelago WebSocket do not mint separate connection identities. `IWeb3IdentityCache.Default` persists one ephemeral identity through `PlayerPrefsIdentityProvider`; the same cache instance is injected into `PulseContainer` for its signed auth chain and into `IArchipelagoIslandRoom.NewDefault` for the WebSocket challenge. On every WebSocket handshake, ws-connector publishes `peer.{address}.connect` with that session address and comms-gatekeeper re-sends the session-addressed island assignment. Those iteration-1 server changes, including takeover/parking support, must be deployed and verified before disabling client heartbeats. No client heartbeat is required for socket reconnect or forced fresh-handshake recovery.
