...
| `get_scene_content_stats` | — | Current scene's content stats (entities, triangles, bodies, geometries, materials, textures, shader variants, colliders, external content) with the documented soft-limit caps for its parcel count; triggers a fresh counting pass |
