...
- **`ui_drag` fails with "the drag panned the camera instead of dragging"** — the pointer was over the world, not over UI, and a held button dragged across the world is the camera-pan gesture. Nothing was dragged and the camera turned. To sweep a held pointer across the world (painting, held drags) use `sweep_pointer`, which is the same gesture a human performs.
