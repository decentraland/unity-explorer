...
- **A device drag over the world drags nothing, and `ok` does not promise otherwise** — a device gesture verifies no target: it reports that the mouse states were replayed. When the held button engages the camera pan (`Camera.TemporalLock` is the left button, so the cursor turns to `Panning` mid-gesture) the call fails with "the drag panned the camera instead of dragging"; when the pan does not engage — the pan needs a free cursor, a dirty cursor position and a pointer off UI — the same call returns a bare `ok` having dragged nothing in the world (measured over a scene's stroke canvas: `ok`, no stroke, camera unmoved). With the cursor locked, any pointer gesture fails up front. To sweep a held pointer across the world (painting, held drags) use `sweep_pointer`, which is the same gesture a human performs.
