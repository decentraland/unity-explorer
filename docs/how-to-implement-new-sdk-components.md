...
1. Merge Protocol repo PR
2. Update both `js-sdk-toolchain` repo PR and `unity-explorer` repo PR to use the updated `@dcl/protocol@next` package
...
> **NOTE:** unity-explorer needs to always use the `@next` protocol package (published from the protocol [`main`](https://github.com/decentraland/protocol/tree/main) branch), otherwise the Unity project won't even compile due to missing component files, etc.
...
**IMPORTANT**: After merging a `@dcl/protocol` PR, we must make unity-explorer point to the updated `@dcl/protocol@next` package (and NOT the PR test package) before merging the unity-explorer PR.
