...
- **A scene measuring "global" input with `inputSystem.isTriggered(action, type)` and no entity is not measuring the scene-root broadcast.** The SDK answers that form from *every* entity's `PointerEventsResult`, so an entity-bound event satisfies it too. A scene that must distinguish the two halves of the fan-out (e.g. to show entity-bound input suppressing the broadcast) has to read `engine.RootEntity` explicitly. Cost us a false "suppression is broken" finding once — check the scene's own measurement before blaming the client.
