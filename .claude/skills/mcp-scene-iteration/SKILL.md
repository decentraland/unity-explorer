...
- `press_input` sends any SDK input action (primary/secondary/action_3…) with the real-key fan-out. **Unaimed it always reaches the scene root, never an entity** — the entity-bound half needs the reticle on the target, and the reticle follows the OS cursor, which a driver never holds over anything (verified 2026-08-28: `look_at` + unaimed `press_input` cannot produce an entity-bound edge). Pass `entityId` or an `x/y/z` aim point to land it entity-bound; the result then reports the target, distance and hover text like a click does. It does NOT move the player — combine with `walk` when a scene expects movement keys.
- **A scene measuring "global" input with `inputSystem.isTriggered(action, type)` and no entity is not measuring the scene-root broadcast.** The SDK answers that form from *every* entity's `PointerEventsResult`, so an entity-bound event satisfies it too. A scene that must distinguish the two halves of the fan-out has to read `engine.RootEntity` explicitly — check the scene's own measurement before blaming the client.
- **An entity-bound edge does NOT suppress the scene-root broadcast** (measured 2026-08-28 against a scene reading `engine.RootEntity`, so the measurement is sound). An aimed `press_input` logs the entity-bound hit *and* increments the scene-root counter on the same frame, whether aimed by `x/y/z` or by `entityId`; every successful `click_entity`/`click_at` does the same. This contradicts the claim in `PressInputTool.cs` ("suppresses the scene-root broadcast for that frame") and `SyntheticPointerEventSystem.cs` ("exactly as a real key press would") — the synthetic layer does not implement suppression, it assumes the production pipeline does it. Do not design a scene demo around the suppression until that is settled Explorer-side.
- `look_at` reports `aimErrorDegrees`: aim residual after the camera settled. It is ~0 normally; a large value with a warning means a rig limit stopped it — in third person the pitch clamps at about ±54°, so a steeply elevated target needs `set_camera_mode first_person`, or backing away so the angle is shallower. Feed real target coordinates; do not inflate `y` to compensate.
...
- **The virtual-device path (`device:true`, and `ui_drag`'s device mode) drives the client UI stack, not UI Toolkit scene panels.** Scene UI consumes events sent to its elements, so `ui_click device:true` on an SDK element reports that the element observed nothing; use the semantic path for scene UI and keep the device path for `stack:ugui`. `ui_drag` handles this itself: a drag starting inside the scene UI is delivered semantically (result says `path: "sdk"`).
...
- **`move_to`'s `lookAtX/Y/Z` turns the avatar, not the camera orbit.** Clicks and hovers are camera-origin rays, so a stale orbit yaw from an earlier `look_at` can put an unrelated entity in the line and the click lands on the wrong target (observed: hit the neighbouring box instead of the intended one, 4 m off-axis). Always `look_at` the aim point immediately before `click_entity`/`hover_entity`; `move_to(lookAt…)` is not a substitute.
- **`move_to` is a teleport and scenes notice.** It fires the scene's movement observers as a superhuman burst (e.g. `distance=16m over 0.05s avgSpeed=320m/s`), and a teleport that leaves one trigger volume and enters another can deliver `enter(B)` **before** `exit(A)` — B's exit handler then clears state A just installed, so a scene-wide `InputModifier` silently ends up cleared while the player stands inside the zone that set it. Use `walk` when the scene watches movement or trigger boundaries; re-enter a zone on foot before trusting its state.
