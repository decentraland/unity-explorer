...
- **`Camera.TemporalLock` is bound to the left mouse button**, so a held-button pointer drag over the world (not over UI) turns the cursor to `Panning`, and `PlayerOriginatedRaycastSystem` resets its raycast while panning. A virtual-device drag in the world therefore pans the camera exactly as a human's does — the gesture system re-checks the captured cursor *every* frame and fails with that reason rather than reporting a drag it did not deliver. World sweeps are `sweep_pointer` (press → camera look → release), never a device drag.
