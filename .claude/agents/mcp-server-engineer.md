...
| Tools | `Tools/*.cs` (one class per tool, **29**) | The agent-facing surface. `Tools/UiAddressArgs.cs` (the `ui_*` family's element address) and `Tools/PointerArgs.cs` (the world-pointer tools' button + aim → `PointerAim`) are shared arg parsing, not tools |
...
The input-simulating tools are **thin front-ends**: `WalkTool`, `LookAtTool`, `CameraLookTool`, `ClickEntityTool`, `ClickAtTool`, `HoverEntityTool`, `PressInputTool` take a `SyntheticInputAgent`; `UiListTool`, `UiClickTool`, `UiSetTextTool`, `UiScrollTool`, `UiDragTool` take a `UiAutomationServices`. They parse/validate args, call one facade method, and shape the result — pointer results through `SyntheticPointerResult.ToJson()`, the one wire shape the AltTester probe emits too. No ECS plumbing lives in them.

### Synthetic input layer — `Explorer/Assets/DCL/SyntheticInput/` (assembly `DCL.SyntheticInput`)

| Piece | Path | Role |
|---|---|---|
| Driver facade | `SyntheticInputAgent.cs`, `PointerAim.cs` | Single entry point for world/avatar input, main-thread only: `WalkAsync`, `CameraLookAsync`, `LookAtAsync`, `ClickAsync`/`PointerDownAsync`/`PointerUpAsync`, `SweepAsync`, `HoverAsync`, `GlobalInputAsync`. Pointer gestures take a `PointerAim` (`AtEntity`/`AtWorldPoint`/`AtScreenPoint`/`None`; the default value is aimless, so no sentinel can leak). **Owns the timeouts** (`COMPLETION_GRACE_SEC`) — a request the simulation never completed is abandoned and reported as timed out |
...
| Systems | `Systems/` (folded into `DCL.Plugins` via `.asmref`) — `SyntheticInputPlugin`, `SyntheticMovementInputSystem` (`InputGroup`, after `UpdateInputMovementSystem` + `UpdateInputJumpSystem`), `SyntheticCameraLookSystem` (`InputGroup`, after `UpdateCameraInputSystem`), `SyntheticPointerEventSystem` (`PresentationSystemGroup`, before `PlayerOriginatedRaycastSystem`) with `SyntheticPointerDiagnostics` (the pure verdict functions it reads the pipeline state through), `UiVirtualDeviceGestureSystem` (`InputGroup`) | Per-frame delivery, each ordered against the production system it piggybacks on |
| UI simulation | `UiSimulation/` — `UiAutomationServices` (session root, one instance), `UiDiscovery`, `UiElementAddress`, `UiOcclusion`, `UiInteractionSimulator`, `SdkUiResolver`, `AutomationVirtualDevices`, `UiDeviceGestureRequest`, `UiScreenGeometry` | Two paths: **semantic** (resolve element → synthesize its events, after an occlusion pre-check) and **virtual devices** (`DclAutomationMouse`/`DclAutomationKeyboard` replayed one state per frame for positional fidelity) |
...
The old "zero production interaction code changed" claim is **no longer true** — the layer needed real seams, and two of them (hover leave, pointer feed) change what real input does:
...
| Global-input broadcast | `Interaction/Systems/PrepareGlobalInputEventsSystem.cs` | Appends the synthetic button edges to the scene-root broadcast buffer after the real ones (deduped against a real same-frame press of the same action; a targeted edge is never appended), and is pinned after `PlayerOriginatedRaycastSystem` / before `ProcessPointerEventsSystem`. Suppression of the broadcast itself is the production one (`WritePointerEventResultsSystem`, consumption time) — a production-time rework exists as a patch for its own PR, see the PR description. `PrepareGlobalInputEventsSystemShould` |
| Hover leave | `Interaction/PlayerOriginated/Utility/HoverFeedbackUtils.cs`, `HoverStateComponent.HoverEnterIssued` | A leave is issued whenever the ending hover *issued an enter* (a flag set on the frame the entity became hovered and kept for the hover's lifetime), and is no longer re-qualified against the frame it ends on (that ray points elsewhere; tight-range targets stayed hovered forever). The flag, not `IsAtDistance`, is the gate: `IsAtDistance` follows the last entry declared and the current frame's range. `HoverFeedbackUtilsShould`, `ProcessPointerEventsSystemShould` |
| Cursor warps + pointer position | `Input/Systems/UpdateCursorInputSystem.cs`, `Character/CharacterCamera/Components/SyntheticCursorOverride.cs` | The system reads a frame-stamped `SyntheticCursorOverride` component from the camera entity — installed beside `CursorComponent` by the automation systems' `Initialize`, absent in retail — which suppresses OS-cursor warps while a virtual-device pointer gesture runs **and supplies the pointer position** for it (and for a parked press). The system otherwise reads one cached `Mouse` (`InputSystem.GetDevice<Mouse>()` in the ctor), which never resolves the automation device — so the injected pointer drove the UI stack (action-driven) while `CursorComponent.Position`, and the world reticle ray built from it, stayed on the OS cursor. Real input is untouched when nothing is asserted |
...
syntheticInputEnabled = FeaturesRegistry.IsEnabled(FeatureId.McpServer)
    ...
    → probes installed via their static latch (#if ALTTESTER), then SyntheticInputPlugin
...
- **Two pointer feeds, one position.** The world reticle ray is built from `CursorComponent.Position`, and `PrimaryPointerInfo.WorldRayDirection` — what scenes sample for a held sweep — from `IExposedCameraData.PointerScreenPosition`, which is that same position copied once per frame. A synthetic *aim* post steers the reticle ray only on the frame it is posted and never moves the pointer, so a scene sampling `PrimaryPointerInfo` sees nothing from a bare `click_at`/`click_entity` aim; what carries a driver's pointer through to a scene is the parked press (`SyntheticPointerHold` → `SyntheticCursorOverride`), and turning the camera under it is what sweeps the sampled ray.
