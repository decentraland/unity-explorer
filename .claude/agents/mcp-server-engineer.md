...
Everything lives in `Explorer/Assets/DCL/McpServer/`, its own `DCL.McpServer` assembly (root asmdef). Three subfolders are folded into other assemblies via `.asmref`: `Systems/` → `DCL.Plugins` (GUID `fc4fd35fb877e904d8cedee73b2256f6`; `DynamicWorldContainer` lives there), `Tests/` → `DCL.EditMode.Tests` and `Tests/PlayMode/` → `DCL.PlayMode.Tests`.

| Piece | Path | Role |
|---|---|---|
| Plugin | `Systems/McpServerPlugin.cs` | Builds the tool registry in `InjectToWorld` (needs `GlobalPluginArguments.PlayerEntity/SkyboxEntity`), starts/disposes the server, wires the scene-log tap (`DiagnosticsContainer.AddDebugConsoleHandler`). Takes `McpNetworkLogBuffer?` — null means no HTTP capture, and `GetNetworkLogTool` is then left out of `tools/list` rather than answering empty. `InjectToWorld` is re-entrant (disposes the previous server/screenshot tool first); `Dispose` also calls `McpKeyboardInput.Reset()` so no key stays held |
| Core (transport + protocol + contract) | `Core/McpHttpServer.cs`, `McpJsonRpcDispatcher.cs`, `McpTool.cs`, `McpToolsRegistry.cs`, `McpToolResult.cs`, `McpToolAnnotations.cs`, `McpJsonSchema.cs`, `McpWireEnum.cs`, `McpEcsRequest.cs`, `McpCallableAttribute.cs` (behind `MCP_TEST_AUTOMATION`: the opt-in marker without which `call_static_method` resolves nothing — no method in the repo carries it yet, and it cannot be applied from an assembly `DCL.McpServer` references, e.g. `SceneRunner.Scene`) | `HttpListener` on `http://127.0.0.1:{port}/unity-explorer-mcp` (the URL template is single-sourced in `IDecentralandUrlsSource.LOCAL_MCP_ENDPOINT_URL`); POST → dispatch, GET → 405, Origin allowlist, mandatory `Content-Length` with a 1 MB body cap (`MAX_BODY_BYTES`); JSON-RPC 2.0 over Streamable HTTP (spec 2025-06-18), tools-only capability |
| Tools | `Tools/*.cs` (one class per tool, 27: 17 that ship everywhere, plus 10 behind `MCP_TEST_AUTOMATION` of which 3 also need `--mcp-reflection`) | The agent-facing surface |
...
| Utils | `Utils/SceneLogBuffer.cs`, `JObjectExtensions.cs`, and behind `MCP_TEST_AUTOMATION` `UiAutomation.cs`, `UiElementPath.cs`, `McpKeyboardInput.cs`, `ComponentProperty.cs` | Log tap buffer, args parsing; the client-UI hierarchy walk + `EventSystem` hit-test + dispatch, the pure path rules, key-name parsing, reflected component reads/writes |

Registration: `DynamicWorldContainer.CreateAsync`, gated on `FeaturesRegistry` `FeatureId.MCP_SERVER` = `appArgs.HasFlag(MCP) || appArgs.HasFlag(MCP_PORT)` — so `--mcp-port` alone implies `--mcp` (presence check; an invalid port value still enables the server and falls back to 8123). `FeatureId.McpReflection` (`McpServer && appArgs.HasFlag(MCP_REFLECTION)`) is the second gate on the three reflection tools, read inside the `MCP_TEST_AUTOMATION` block. The same `FeatureId.McpServer` block also calls `staticContainer.WebRequestsContainer.EnableMcpNetworkLog()` before constructing the plugin — HTTP capture is off until it does, so keep the call and the plugin construction in one gate rather than re-deriving the flag expression in `BootstrapContainer` (where `FeaturesRegistry.Instance` does not exist yet). **All three flags are command-line only, and that is the shipped design, not an omission**: `DeepLinkAllowlist` is deny-by-default and lists none of them, so a `decentraland://` link cannot start the server. `mcp-reflection` must stay out of that allowlist unconditionally — arbitrary in-process reads, writes and invocation must not be reachable from a crafted link. If `mcp` is ever allowlisted to fix the creator flow, keep it loopback-gated and do not weaken `MCP_TEST_AUTOMATION` in the same change; it is a product decision (SEC-019/020), so do not make it unasked. Log category: `ReportCategory.MCP`.

Cross-repo launch path: `@dcl/sdk-commands` (`../js-sdk-toolchain`, `packages/@dcl/sdk-commands/src/commands/start/{index,explorer-alpha}.ts`) forwards `--mcp` / `--mcp-port` from `npm run start` into the `decentraland://` deep link (`mcp=true` / `mcp-port=<n>`), plus arbitrary params after a second standalone `--`. **That path is dead on arrival today** — the allowlist drops both keys, so `npm run start -- --mcp` starts a client with no server and no error; serve with `--no-client` and put the flags on the client's own command line. Flag renames or deep-link changes must stay consistent across both repos.
...
EditMode tests exist in `Tests/` (folded into `DCL.EditMode.Tests` via asmref): dispatcher, registry, result routing, input schema, HTTP server, state tools, pointer-click system, ECS-request helper (`McpEcsRequestShould`), wire-enum mapping (`McpWireEnumShould`), server gating (`McpServerGatingShould`), network-log ring buffer, UI-element path rules, key-name parsing and held-key tracking, component lookup and component-property reads/writes, `call_static_method` resolve/bind/invoke plus its `[McpCallable]` refusal (`CallStaticMethodToolShould`). PlayMode tests in `Tests/PlayMode/` (folded into `DCL.PlayMode.Tests`) cover what needs a live `EventSystem` and a real canvas: `UiHitTestShould` drives `UiAutomation` directly — click and hover on a clear control, refusal on an occluded one, hover-exit of the previous element. Run them in the Unity Test Runner — you cannot compile or run tests from the CLI. Smoke-test the protocol layer with the running client:
...
Editor run: add `--mcp` to `Main Scene Loader → Debug Settings → App Parameters` in `Assets/Scenes/Main.unity` and hit Play. Standalone against a local scene: serve it with `npm run start -- --no-client` in the scene folder, then launch the client yourself with `--realm http://127.0.0.1:8000 --local-scene true --mcp` (`--mcp-port <port>` for another port, plus `--multi-instance` and a distinct dev-server `--port` for side-by-side instances). Do not expect `npm run start -- --mcp` to work — see the cross-repo launch path above. Full launch lines are in `docs/mcp-automation.md`.
