...
Request flow: `HttpListener` accepts on the thread pool → detached `UniTaskVoid` per request → dispatcher parses/routes → `McpTool.ExecuteAsync` hops to the main thread (unless the tool overrides `requiresMainThread` to false — see `GetSceneLogsTool`, which reads only the thread-safe `SceneLogBuffer` and answers even while the main thread is busy or paused) and runs the tool body `ExecuteCoreAsync(JObject, ct)` → heavy encoding (base64) hops back via `DCLTask.SwitchToThreadPool()`.
