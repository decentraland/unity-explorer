...
namespace DCL.Optimization.AdaptivePerformance.Systems
{
    ...
    public class AdaptivePhysicsSettings : ScriptableObject
    {
        ...
        [field: Tooltip("The maximum value that UnityEngine.Time.fixedDeltaTime can be clamped to, preventing the simulation from slowing down excessively.")]
        [field: SerializeField] internal float maxFixedDelta = 0.05f;

        [field: Header("Update Criteria")]
        [field: Tooltip("The minimum interval (in seconds) before another UnityEngine.Time.fixedDeltaTime adjustment can occur, preventing overly frequent changes.")]
        [field: SerializeField] internal float changeCooldown = 1; // sec
        [field: Tooltip("The minimum number of frame time samples required before UnityEngine.Time.fixedDeltaTime adjustments are considered, ensuring enough data is collected.")]
        [field: SerializeField] internal float minFrameTimeAmount = 30;

        [field: Header("Adaptive Change Buffer Range")]
        [field: Tooltip("How much (in ms) the median frame time can exceed the current UnityEngine.Time.fixedDeltaTime before it’s considered \"too high\" and triggers a recalculation. Larger values allow more fluctuation before recalculation.")]
