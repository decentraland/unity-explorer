...
namespace DCL.ApplicationGuards
{
    public class SystemSpecUtilsShould
    {
        ...
        /// <summary>
        ///     The requirements payload is feature-flag served and can be absent (a --base-domain deployment whose
        ///     flags backend does not publish it). Every check must fail open rather than throw on the default
        ///     struct's null members: acceptability checks pass, the integrated-GPU classifier reports nothing.
        /// </summary>
        [Test]
        public void FailOpenWhenRequirementsPayloadIsMissing()
        {
            FeatureFlagsConfiguration.Reset();
