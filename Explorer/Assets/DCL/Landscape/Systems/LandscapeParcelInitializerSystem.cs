...
namespace DCL.Landscape.Systems
{
    ...
    public partial class LandscapeParcelInitializerSystem : BaseUnityLoopSystem
    {
        ...
        {
            var genesisCityOffset = new Vector3(GENESIS_HALF_PARCEL_WIDTH * PARCEL_SIZE, 0,
                GENESIS_HALF_PARCEL_WIDTH * PARCEL_SIZE);
            ...
            for (var x = 0; x < 8; x++)
            {
                for (var y = 0; y < 8; y++)
                {
                    ...
                    var coord = new Vector3(posX, 0, posZ) - genesisCityOffset + quadCenter + mapTextureMargins +
                                zFightPrevention;

                    var groundTile = Object.Instantiate(landscapeData.mapChunk, landscapeParentObject, true);
                    ...
                    var satelliteRenderer = groundTile.GetComponent<Renderer>();
