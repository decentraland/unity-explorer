...
namespace DCL.Landscape
{
    public class SatelliteFloor
    {
        ...
        public void Create(MapRendererTextureContainer textureContainer)
        {
            ...
            initialized = true;
            SwitchVisibility();
            SwitchDistanceClip(landscapeData.drawTerrain);
        }

        public void OnTerrainVisibilityChanged(bool newValue)
        {
            SwitchDistanceClip(newValue);
        }
        ...
        private void SwitchDistanceClip(bool enabled)
        {
            var mapChunkRenderer = landscapeData.mapChunk.GetComponent<Renderer>();
            Material satelliteFloorMaterial = mapChunkRenderer.sharedMaterial;

            if (enabled)
                satelliteFloorMaterial.EnableKeyword("_DISTANCE_CLIP_ON");
            else
                satelliteFloorMaterial.DisableKeyword("_DISTANCE_CLIP_ON");
        }
