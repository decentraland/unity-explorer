...
namespace DCLServices.MapRenderer.MapLayers.Atlas.SatelliteAtlas
{
    public class SatelliteChunkController : IChunkController
    {
        ...
        private async UniTask<UnityWebRequest> GetTextureFromWebAsync(string url, int maxAttempts = 3, CancellationToken ct = default)
        {
            ...
            try { webRequest = await webRequestController.GetTextureAsync(url, cancellationToken: linkedCts.Token); }
...
#else
            webRequest = await webRequestController.GetTextureAsync(url, cancellationToken: linkedCts.Token);
#endif
