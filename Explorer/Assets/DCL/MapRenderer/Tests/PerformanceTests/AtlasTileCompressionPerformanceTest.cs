...
{
    /// <summary>
    /// Falsifies fix #19 — "Block-compress satellite/parcel atlas tiles to cut lifetime map texture memory ~4x".
    /// <para>
    ...
    /// <c>TextureFormat.RGBA32</c>, NO-mip <see cref="Texture2D"/> (GetTextureWebRequest.cs:79-82) that is
    /// then retained for the process lifetime (satellite in <c>currentOwnedTexture</c> +
    /// <c>MapRendererTextureContainer.AddChunk</c>; parcel in <c>currentOwnedTexture</c> + the sprite).
    /// A full atlas holds dozens of such tiles at 4 bytes/pixel. The fix guard-and-compresses the
    /// downloaded tile in-place before <c>Sprite.Create</c>:
    /// <code>
    ...
    /// </para>
    /// <para>
    /// This test reproduces that exact buffer-path tile and the exact guard-and-compress expression the
    /// patch applies (kept in <see cref="CompressAtlasTile"/> verbatim), then measures the fix spec's
    /// primary metric — the sum of <see cref="Profiler.GetRuntimeMemorySizeLong(UnityEngine.Object)"/>
    /// over the retained tile textures — before vs after. The pass gate mirrors the spec: &gt;=60%
    /// reduction. If the compression were absent or ineffective (fix reverted) the retained bytes stay at
    /// the RGBA32 footprint and the &gt;=60% assert fails. A second test exercises the multiple-of-4
    /// guard: a 510x510 tile MUST be left untouched (still RGBA32), falsifying an over-aggressive change
    /// that would corrupt non-conforming tiles.
    /// </para>
    /// <para>
    /// Note: the two production controllers take the compressed tile through <c>Sprite.Create</c> and a
    /// <c>SetTexture</c> GPU binding only (verified: the sole <c>GetChunk</c> consumer is
    ...
    {
        // Satellite tiles are square JPGs and parcel tiles are chunkSize-square; 512 is the representative
        // atlas tile resolution and is a multiple of 4 (passes the fix's dimension guard).
        private const int TILE_RESOLUTION = 512;

        // Full satellite atlas is 8x8 chunks (MapRendererTextureContainer.IsComplete() => Count >= 64).
        ...
        /// <summary>
        /// VERBATIM copy of the guard-and-compress the fix inserts into both chunk controllers. Kept here
        /// so this test falsifies the actual patch logic (drift the guard and the asserts below break).
        /// </summary>
        ...
        /// <summary>
        /// The falsifier: an atlas-sized batch of retained RGBA32 tiles must shrink by &gt;=60% once the
        /// fix's compression runs. With the fix reverted (no Compress) the delta is 0% and this fails.
        /// </summary>
        [Test]
        [Performance]
        public void RetainedAtlasTileMemoryDropsAtLeast60Percent()
        {
            ...
            Assert.That(reduction, Is.GreaterThanOrEqualTo(0.60),
                ...
                $"({beforeBytes} -> {afterBytes} bytes); fix #19 requires >=60% (was the tile compressed?).");

            // Every conforming tile must actually be in a block-compressed format now, not RGBA32.
            foreach (Texture2D tile in tiles)
        ...
        /// <summary>
        /// The safety falsifier: the multiple-of-4 guard must skip a 510x510 tile so a non-conforming tile
        /// is never fed to Texture2D.Compress (which requires 4-aligned dims). An over-aggressive change
        /// that drops the guard corrupts / errors on such tiles — this asserts the tile is untouched.
        /// </summary>
        [Test]
        [Performance]
