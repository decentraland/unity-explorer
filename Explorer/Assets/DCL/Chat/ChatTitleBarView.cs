...
namespace DCL.Chat
{
    public class ChatTitleBarView : MonoBehaviour, IViewWithGlobalDependencies
    {
        ...
        public void Initialize()
        {
            ...
            communityChannelContainer.LeaveCommunityRequested += OnCommunityContextMenuLeaveCommunityRequested;
            isInitialized = true;
        }
        ...
        public void SetupCommunityView(IThumbnailCache thumbnailCache, string communityId, string communityName, bool isOwnerOfCommunity, string? thumbnailUrl, CommunityTitleView.OpenContextMenuDelegate openContextMenuAction, CancellationToken ct)
        {
            nearbyChannelContainer.SetActive(false);
            memberCountObject.SetActive(true);
            profileView.gameObject.SetActive(false);
            communityChannelContainer.gameObject.SetActive(true);
            communityChannelContainer.SetupAsync(thumbnailCache, communityId, communityName, isOwnerOfCommunity, thumbnailUrl, openContextMenuAction, ct).Forget();
        }
        ...
        private void OnCommunityContextMenuLeaveCommunityRequested()
        {
