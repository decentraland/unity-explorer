...
namespace DCL.Tests.PlayMode.PerformanceTests
{
    ...
    public class ChatBubblePreferredWidthPerformanceTest
    {
        ...
        public void SetUp()
        {
            ...
            // SetMessageData -> ChatMessage/ChatEntryMessageBubbleElement reach OfficialWalletsHelper.Instance, which
            // throws until initialized. Its constructor reads FeatureFlagsConfiguration.Instance, so init it after that.
            ...
            // The chat-entry hierarchy is instantiated inactive (hence GetComponentInChildren(true)). While inactive the
            // TMP text never Awakes, so the ForceMeshUpdate inside the guarded SetMessageContent early-returns without
            // clearing havePropertiesChanged — making the no-redundant-reparse assertion below unobservable. Activate the
            // chain up to the (already active) canvas so the flag is actually cleared once the text is bound clean.
            for (Transform t = bubble.messageContentElement.messageContentText.transform; t != null && t != canvas.transform; t = t.parent)
