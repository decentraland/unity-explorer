        ...
        public object GetOwnUserData()
        {
            async UniTask<GetUserDataResponse> GetOwnUserDataAsync(CancellationToken ct)
            {
                try
                {
                    ...
                    Profile? profile = await profileRepository.GetAsync(identity.Address, ct, IProfileRepository.FetchBehaviour.EnforceSingleGet | IProfileRepository.FetchBehaviour.DelayUntilResolved);
