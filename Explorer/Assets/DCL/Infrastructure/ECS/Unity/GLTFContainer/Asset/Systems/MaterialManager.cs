...
namespace DCL.Rendering.RenderSystem
{
    public class MaterialManager
    {
        ...
        private List<PerMaterial> perMaterials;
        private SortedDictionary<uint, Material> materialSortedDictionary = new SortedDictionary<uint, Material>();

        public static readonly int RSUV_PerMaterialBuffer = Shader.PropertyToID("_GPUBuffer_PerMaterial");

        public MaterialManager()
        {
            GPUBuffer_PerMaterial = new GraphicsBuffer(GraphicsBuffer.Target.Structured, GraphicsBuffer.UsageFlags.LockBufferForWrite, UInt16.MaxValue, Marshal.SizeOf(typeof(PerMaterial)));
            Shader.SetGlobalBuffer(RSUV_PerMaterialBuffer, GPUBuffer_PerMaterial);
            perMaterials = new List<PerMaterial>();
        }
        ...
        private Material SRUV_Adjustment(Renderer renderer)
        {
            ...
            newMaterial.SetVector("_BaseColor", Vector4.zero);
