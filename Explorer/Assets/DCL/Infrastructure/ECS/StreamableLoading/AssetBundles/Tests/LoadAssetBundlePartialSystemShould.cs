...
namespace ECS.StreamableLoading.AssetBundles.Tests
{
    ...
    public class LoadAssetBundlePartialSystemShould :  UnitySystemTestBase<LoadAssetBundleSystem>
    {
        ...
        public async Task ParallelABLoadsWithoutCacheShould()
        {
            ...
            IWebRequestController webRequestController = new WebRequestController(IWebRequestsAnalyticsContainer.DEFAULT, new IWeb3IdentityCache.Default(), new RequestHub(Substitute.For<IDecentralandUrlsSource>()), ChromeDevtoolProtocolClient.NewForTest(), new ElementBinding<ulong>((ulong)totalBudget), totalBudget);
            system = CreateSystem(webRequestController, diskCachePartials);
            system.Initialize();
