...
{
    /// <summary>
    ///     Verifies <see cref="ReusableDelay" /> stays effectively allocation-free in steady state (unlike
    ///     Task.Delay(int, CancellationToken), which allocates a promise, a timer, and a per-call
    ///     CancellationTokenRegistration on every await), reacts to cancellation well under the armed interval,
    ///     and completes exactly once under a timer-fire-vs-cancel race.
    /// </summary>
    [Category("Performance")]
    public class ReusableDelayShould
    {
        ...
        public IEnumerator UpdateLoopTickDelay_ReusableWait_AllocFreeSteadyStateAndCancelPrompt() =>
            UniTask.ToCoroutine(async () =>
            {
                ...
                using var delay = new ReusableDelay();
                ...
                Measure.Custom(new SampleGroup("ReusableDelay_bytesPerAwait", SampleUnit.Byte), Math.Max(reusableBytes, 0));
                ...
                using var cancelDelay = new ReusableDelay();
                ...
                for (var i = 0; i < 500; i++)
                {
                    ...
                    using var raceDelay = new ReusableDelay();
                ...
                Assert.AreEqual(0, otherFaults, "the value-task-source must complete exactly once (no double-completion faults)");
            });

        private static void DriveOnce(ReusableDelay delay, int ms)
        {
            System.Runtime.CompilerServices.ValueTaskAwaiter awaiter = delay.Delay(ms).GetAwaiter();
