...
{
    /// <summary>
    /// Falsifies perf fix #14: <see cref="CharacterEmoteSystem"/>'s <c>UpdateEmoteTags</c> query must
    /// poll the native Mecanim animator state (<c>IAvatarView.GetAnimatorCurrentStateTag</c>) only for
    /// avatars that are actually emoting — not for every entity carrying a
    /// <see cref="CharacterEmoteComponent"/>. The guard early-outs when
    ...
    /// (legacy) emote reference. Pass criterion: poll count == <c>emoting</c>. The pre-fix code polls
    /// all <c>n</c> avatars and fails the assertion. A <c>Measure.Method</c> also records the guarded
    /// full-Update cost over the crowd for the timing dimension.
    /// </para>
    /// <para>
    ...
    public class CharacterEmoteSystemPerformanceTest : UnitySystemTestBase<CharacterEmoteSystem>
    {
        ...
        public void SetUp()
        {
            ...
            avatarView.GetAnimatorCurrentStateTag(Arg.Any<string>()).Returns(_ =>
            {
                animatorTagPolls++;
                return 0;
            });
        ...
        private void PopulateCrowd(int n, int emoting)
        {
            for (int i = 0; i < n; i++)
            {
                ...
                if (i < emoting)
                    // A live emote reference keeps the guard from early-outing, so the avatar is polled.
        ...
        public void UpdateEmoteTags_PollsOnlyEmotingAvatars(int n, int emoting)
        {
            ...
            // Falsification: exactly one native animator poll per emoting avatar, zero for idle ones.
            animatorTagPolls = 0;
            system!.Update(0f);
            ...
            Assert.AreEqual(emoting, animatorTagPolls,
                ...
                $"({emoting} emoting). The guard must limit native polls to emoting avatars; the pre-fix code polls all {n}.");

            // Timing dimension: full guarded Update over the crowd.
