...
namespace DCL.AvatarRendering.AvatarShape.Components
{
    ...
    internal class RemoteAvatarPipeline : IDisposable
    {
        ...
        // Per-avatar count of matrices the calculation job must produce, indexed by job slot. Kept in
        // sync every frame from the authoritative AvatarCustomSkinningComponent.BoneCount (the exact count
        // ComputeSkinning uploads). Defaults to bonesArrayLength so any slot the per-frame refresh has not
        // written falls back to the original full-stride behaviour rather than under-computing.
