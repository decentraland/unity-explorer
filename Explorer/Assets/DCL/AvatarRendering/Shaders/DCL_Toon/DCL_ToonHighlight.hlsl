...
VertexOutput vert (VertexInput v)
{
    ...
    #ifdef _DCL_COMPUTE_SKINNING
        float4 vVertex = float4(_GlobalAvatarBuffer[_lastAvatarVertCount + _lastWearableVertCount + v.index].position.xyz, 1.0);
        o.pos = UnityObjectToClipPos(float4(vVertex.xyz + signVar*normalize(vVertex)*Set_Outline_Width, 1));
    #else
