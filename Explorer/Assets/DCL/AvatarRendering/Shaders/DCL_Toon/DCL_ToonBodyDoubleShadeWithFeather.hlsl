float4 fragDoubleShadeFeather(VertexOutput i, half facing : VFACE) : SV_TARGET 
{
    ...
    mainLightColor *= (mainLight.shadowAttenuation);
    int nMainTexArrID = _MainTexArr_ID;
    float2 uv_maintex = TRANSFORM_TEX(Set_UV0, _MainTex);
    float4 _MainTex_var = SAMPLE_MAINTEX(uv_maintex,nMainTexArrID);
    ...
    // HACK for testing
    float3 lightColor = originalLightColor;//lerp(max(defaultLightColor, originalLightColor), max(defaultLightColor, saturate(originalLightColor)), _Is_Filter_LightColor);
        ...
        int pixelLightCount = GetAdditionalLightsCount();
            ...
            for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); lightIndex++)
            {
                ...
                // if (iLight != i.mainLightID)
                {
                    ...
                    UtsLight additionalLight = GetUrpMainUtsLight(inputData.shadowCoord, float4(inputData.positionWS.xyz, 1.0f));
                    ...
                    additionalLightColor *= (additionalLight.distanceAttenuation * additionalLight.shadowAttenuation);
