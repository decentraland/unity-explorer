...
namespace DCL.ParcelsService
{
    public partial class TeleportController : ITeleportController
    {
        ...
        public async UniTask TeleportToSceneSpawnPointAsync(Vector2Int parcel, AsyncLoadProcessReport loadReport,
            CancellationToken ct)
        {
            if (retrieveScene == null)
            {
                AddTeleportIntentQuery(world,
                    new PlayerTeleportIntent(ParcelMathHelper.GetPositionByParcelPosition(parcel), parcel));
            ...
            {
                loadReport.CompletionSource.TrySetException(e);
            }
        }

        public async UniTask TeleportToParcelAsync(Vector2Int parcel, AsyncLoadProcessReport loadReport,
            CancellationToken ct)
        {
            if (retrieveScene == null)
            {
            ...
            var characterPos = ParcelMathHelper.GetPositionByParcelPosition(parcel);
            var sceneDef = await retrieveScene.ByParcelAsync(parcel, ct);
