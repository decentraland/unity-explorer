...
namespace DCL.VoiceChat.Nearby.Systems
{
    ...
    ///     <see cref="StreamingAudioComponent"/> + <see cref="InAudibleRangeTag"/>) the system
    ///     iterates the entity's own <c>SidsSnapshot</c> and materializes an audio-source entity
    ///     per <c>(walletId, sid)</c> pair that does not yet have one. Throttled to a fixed budget
    ///     per frame so a crowd ramp-up does not spike a single tick.
    /// </summary>
    ...
    public partial class NearbyAudioBindingSystem : BaseUnityLoopSystem
    {
        ...
        private void CollectPendingCreations(Entity avatarEntity, in Profile profile, in StreamingAudioComponent streaming)
        {
            ...
            // Sids ride on the entity (StreamingAudioComponent.SidsSnapshot is a reference to the
            // registry's COW array). foreach over a string[] lowers to indexed for-loop in IL —
            // no enumerator allocation, no registry call on the per-avatar hot path.
        ...
        private void DrainPendingCreations()
        {
            ...
            foreach ((Entity avatarEntity, StreamKey key) in pendingCreations)
            {
                ...
                // Track was unsubscribed between collection and resolve (GetActiveStream); skip to avoid a one-frame ghost source.
