...
{
    public class GPUInstancingService_Old : IDisposable
    {
        ...
        {
            foreach ((GPUInstancingCandidate_Old candidate, GPUInstancingBuffers_Old buffers) in candidatesBuffersTable)
                RenderCandidateIndirect(candidate, buffers);
        }

        private static void RenderCandidateIndirect(GPUInstancingCandidate_Old lodGroup, GPUInstancingBuffers_Old buffersOld)
        {
            ...
            foreach (MeshRenderingData_Old meshData in lod.MeshRenderingDatas)
            {
                ...
                {
                    buffersOld.DrawArgsCommandData[currentCommandIndex].indexCountPerInstance = meshData.SharedMesh.GetIndexCount(submeshIndex);
                    buffersOld.DrawArgsCommandData[currentCommandIndex].instanceCount = (uint)lodGroup.InstancesBuffer.Count;
                    buffersOld.DrawArgsCommandData[currentCommandIndex].startIndex = meshData.SharedMesh.GetIndexStart(submeshIndex);
                    ...
                    Graphics.RenderMeshIndirect(rparams, meshData.SharedMesh, buffersOld.DrawArgsBuffer, commandCount: 1, currentCommandIndex);
                    currentCommandIndex++;
                }
            }
        }
    ...
    public class GPUInstancingBuffers_Old : IDisposable
    {
