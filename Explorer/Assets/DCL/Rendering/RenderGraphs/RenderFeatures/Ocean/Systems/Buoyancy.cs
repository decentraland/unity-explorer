...
namespace DCL.Rendering.RenderGraphs.RenderFeatures.Ocean
{
    public static partial class Buoyancy
    {
        ...
        private static float _TimeParameters
        {
            get
            {
                if (OceanObject.CustomTime > 0) return OceanObject.CustomTime;
        ...
        public static void Raycast(OceanObject oceanObject, Vector3 origin, Vector3 direction,  bool dynamicMaterial, out RaycastHit hit)
        {
            Raycast(oceanObject.material, oceanObject.transform.position.y, origin, direction, dynamicMaterial, out hit);
        }
        ...
        public static float SampleWaves(UnityEngine.Vector3 position, OceanObject oceanObject, float rollStrength, bool dynamicMaterial, out UnityEngine.Vector3 normal)
        {
            return SampleWaves(position, oceanObject.material, oceanObject.transform.position.y, rollStrength, dynamicMaterial, out normal);
        }
        ...
        private static void SampleWaves(UnityEngine.Vector3 position, Material waterMat, float waterLevel, float rollStrength, bool dynamicMaterial, out UnityEngine.Vector3 offset, out UnityEngine.Vector3 normal)
        {
            ...
            planarPosition.x = position.x - OceanObject.PositionOffset.x;
            planarPosition.y = position.z - OceanObject.PositionOffset.z;
        ...
        /// <returns></returns>
        public static bool CanTouchWater(Vector3 position, OceanObject oceanObject)
        {
            if (!oceanObject) return false;

            return position.y < (oceanObject.transform.position.y + WaveParameters.GetMaxWaveHeight(oceanObject.material));
        }
