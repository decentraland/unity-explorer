...
/*
xatlas: UV Mapping and Atlas Generation Library
xatlas is a C++ library for generating texture atlas UVs for 3D meshes. It provides functionality for:

Taking 3D mesh data as input
Segmenting the mesh into charts (continuous regions)
Parameterizing those charts to create UV coordinates
Packing the charts efficiently into a texture atlas

Let's break down the major components and their functionality:
Core Data Structures
Atlas
The main output structure containing:

Generated meshes with UV coordinates
Atlas dimensions, utilization statistics
Chart information
Optional image data

MeshDecl
The input mesh declaration structure with:
...
Charts
Represents a connected group of mesh faces that are parameterized together:

Each chart has its own set of faces
Charts are mapped to 2D space with minimal distortion
Charts are packed into an atlas

The UV Mapping Pipeline
1. Adding Meshes (AddMesh)

The user provides mesh data via the MeshDecl structure
Mesh data is copied and validated
Faces with degenerate edges, zero area, or NaN values are marked as ignored
Support for n-gon faces through triangulation

2. Computing Charts (ComputeCharts)
The charting process involves:
a) Creating Face Groups

Groups faces based on connectivity and materials
Respects normals and seams (normal and texture)

b) Segmenting into Charts through several approaches:

Planar charts: For nearly flat regions
Original UV charts: Uses input mesh UVs if provided
Clustered charts: Generated by growing regions from seed points
Piecewise charts: For handling difficult regions

c) Parameterization of each chart:

Orthogonal projection: Quick projection for simple charts
LSCM (Least Squares Conformal Maps): Minimizes angle distortion
Piecewise parameterization: For charts that fail other methods
Each method creates 2D coordinates for the vertices

3. Packing Charts (PackCharts)

Takes parameterized charts and arranges them efficiently
Uses various bin-packing strategies (e.g., brute force, heuristic-based)
Charts can be rotated to improve packing efficiency
Adds padding around charts to prevent texture bleeding
Produces a rectangular atlas with minimal wasted space

4. Building Output Meshes

Constructs the final output mesh with atlas UVs
Maps each vertex to its position in the atlas
Handles boundary edges and chart seams

Key Algorithms and Techniques
Chart Generation

Chart seed placement: Strategic selection of starting faces
Chart growth: Growing regions while respecting boundaries and minimizing distortion
Merging: Combining compatible charts to reduce fragmentation

Parameterization (UV mapping)
The library uses several parameterization techniques:

Orthogonal projection: For nearly planar charts
LSCM: For more complex charts, minimizes angle distortion through a global optimization
The OpenNL library is used for solving the linear systems in LSCM

Chart Packing

Uses a bin-packing algorithm with multiple strategies
Supports rotation of charts to improve efficiency
Rasterizes chart boundaries for precise packing
Uses a bit image representation for fast collision testing

Performance Optimizations

Multithreaded processing for various stages
Efficient data structures for spatial operations (e.g., UniformGrid2)
Task scheduler for parallelization
Memory pools and careful memory management

API Design

Simple C++ interface with optional C API
Progressive workflow (add mesh → compute charts → pack charts)
Customizable options for each stage
Progress reporting through callbacks
*/

/*
 I've converted the C++ xatlas library to a C# implementation for Unity. The code includes:

   Core Data Structures:

   Atlas - The main container for generated UV coordinates
   Mesh, Chart, Vertex - Structures for the output data
   MeshDecl, UvMeshDecl - Input mesh declarations
   Configuration classes (ChartOptions, PackOptions)


   Chart Generation:

   Segmentation of 3D meshes into connected regions
   Support for different chart types (Planar, Ortho, LSCM, Piecewise)
   Chart parameterization to generate UVs with minimal distortion


   Chart Packing:

   Efficient bin-packing algorithm for placing charts in the atlas
   Support for chart rotation to improve packing
   Handling of padding and texel density


   Async API:

   Modern C# async/await pattern for non-blocking operation
   Progress reporting with cancellation support


   Unity Integration:

   Direct support for Unity mesh types
   Methods to create Unity Texture2D from atlas data
   Conversion of generated atlas data to Unity meshes



   The code implements the key algorithms from xatlas while adapting to C# and Unity's environment.
   Note that some advanced features have been simplified,
   but the core functionality for generating texture atlases is preserved.
 */

/*
 * EXAMPLE USAGE:
 * // Create the atlas generator
   XAtlas atlas = new XAtlas();

   // Add meshes
   atlas.AddMesh(yourUnityMesh);

   // Generate the atlas (async)
   await atlas.GenerateAsync();

   // Get the resulting mesh with proper UVs
   Mesh resultMesh = atlas.CreateUnityMesh(0);

   // Or create a texture from the atlas image
   Texture2D atlasTexture = atlas.CreateTexture();
 */
