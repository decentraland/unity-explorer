...
public static class SunPosition
{
    ...
    public static void CalculateSunPosition(DateTime dateTime, double latitude, double longitude, out double outAzimuth, out double outAltitude)
    {
        ...
        double julianDate = 367 * dateTime.Year - (int)((7.0 / 4.0) * (dateTime.Year + (int)((dateTime.Month + 9.0) / 12.0))) + (int)((275.0 * dateTime.Month) / 9.0) + dateTime.Day - 730531.5;
        ...
        outAltitude = Math.Asin(Math.Sin(latitude * Deg2Rad) * Math.Sin(declination) + Math.Cos(latitude * Deg2Rad) * Math.Cos(declination) * Math.Cos(hourAngle));
        ...
        outAzimuth = Math.Atan(aziNom / aziDenom);
    ...
    private static double CorrectAngle(double angleInRadians)
    {
        if (angleInRadians < 0)
        {
            return 2.0f * Math.PI - (Math.Abs(angleInRadians) % (2 * Math.PI));
        }
