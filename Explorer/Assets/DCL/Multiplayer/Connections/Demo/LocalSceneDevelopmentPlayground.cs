...
namespace DCL.Multiplayer.Connections.Demo
{
    public class LocalSceneDevelopmentPlayground : MonoBehaviour
    {
        ...
        private async UniTaskVoid LaunchAsync()
        {
            ...
            var totalBudget = 15;
            var webRequests = new WebRequestController(new WebRequestsAnalyticsContainer(), identityCache, new RequestHub(urlsSource), ChromeDevtoolProtocolClient.NewForTest(), new WebRequestBudget(totalBudget, new ElementBinding<ulong>((ulong)totalBudget)));
