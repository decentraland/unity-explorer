    ...
    public static class OutfitsExtensions
    {
        ...
        public static List<string> ToFullWearableUrns(this IEquippedWearables equippedWearables, IWearableStorage wearableStorage, Profile profile)
        {
            ...
            foreach ((string category, var w) in equippedWearables.Items())
            {
                ...
                ReportHub.Log(ReportCategory.OUTFITS, $"<color=orange>[SAVE_OUTFIT_ATTEMPT]</color> Trying to find full URN for short URN: '{potentialItemUrn}'");
                        ...
                        out var registry) && registry.Count > 0)
                {
                    ...
                    ReportHub.Log(ReportCategory.OUTFITS, $"<color=cyan>[SAVE_OUTFIT_SUCCESS]</color> Found mapping! Using full URN: '{potentialItemUrn}'");
                }
                ...
                {
                    ReportHub.Log(ReportCategory.OUTFITS, $"<color=red>[SAVE_OUTFIT_FAILURE]</color> No entry found in ownedNftsRegistry for '{potentialItemUrn}'. Falling back.");
