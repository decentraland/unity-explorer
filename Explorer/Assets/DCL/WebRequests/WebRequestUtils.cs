...
namespace DCL.WebRequests
{
    public static class WebRequestUtils
    {
        ...
        private static readonly ConcurrentDictionary<string, bool> privateHostCache = new ();

        /// <summary>
        ///     True when the URL's host resolves only to loopback, private (RFC 1918), link-local,
        ///     carrier-grade-NAT (RFC 6598, used by VPN overlays), or IPv6 ULA addresses. Content on
        ///     such hosts cannot be reached by public server-side services (e.g. the media converter),
        ///     so it must be fetched directly. The verdict is cached per host; the first call for a
        ///     host performs a blocking DNS resolution.
        /// </summary>
        public static bool IsPrivateNetworkHost(string url)
        {
            ...
            return privateHostCache.GetOrAdd(uri.Host, static host =>
            {
                try
                {
