                ...
                catch (UnityWebRequestException exception)
                {
                    ...
                    if (!canBeRepeated && !envelope.IgnoreIrrecoverableErrors)
                    {
                        ...
                        if (!envelope.CommonArguments.URL.Value.StartsWith("file://", StringComparison.OrdinalIgnoreCase))
                            SentrySdk.AddBreadcrumb($"Irrecoverable exception (code {exception.ResponseCode}) occured on executing {envelope.GetBreadcrumbString(BREADCRUMB_BUILDER.Value)}", level: BreadcrumbLevel.Info);
