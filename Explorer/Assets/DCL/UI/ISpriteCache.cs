...
namespace DCL.UI
{
    ...
    public interface ISpriteCache
    {
        ...
        Sprite? GetCachedSprite(Uri imageUrl);
        ...
        UniTask<Sprite?> GetSpriteAsync(Uri imageUrl, CancellationToken ct = default);
        ...
        /// <param name="ct">Cancellation token.</param>
        /// <returns>The sprite that envelopes the image, or null if there was a problem obtaining the image.</returns>
        UniTask<Sprite?> GetSpriteAsync(Uri imageUrl, bool useKtx, CancellationToken ct = default);
        ...
        void AddOrReplaceCachedSprite(Uri? imageUrl, Sprite imageContent);
