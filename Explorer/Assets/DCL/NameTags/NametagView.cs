...
namespace DCL.Nametags
{
    public class NametagView : MonoBehaviour
    {
        ...
        private static readonly float speakingIconSpacing = NametagViewConstants.DEFAULT_SPEAKING_ICON_SPACING;
        ...
        public void SetUsername(string username, string? walletId, bool hasClaimedName, bool useVerifiedIcon, Color usernameColor)
        {
            ...
            usernameText.rectTransform.sizeDelta = new Vector2(usernameText.preferredWidth, NametagViewConstants.DEFAULT_HEIGHT);
        ...
        public void SetChatMessage(string chatMessage, bool isMention, bool isPrivateMessage, bool isOwnMessage, string recipientValidatedName,
            string recipientWalletId, Color recipientNameColor, bool isCommunityMessage, string communityName)
        {
            cts.SafeCancelAndDispose();
            cts = new CancellationTokenSource();
            ...
            else if (isCommunityMessage) { communityNameText.text = communityName; }
        ...
        [BurstCompile]
        private static float CalculateTransparency(float distance, float maxDistance, float fullOpacityMaxDistance, float previousDistance, float distanceThreshold,
        ...
        private async UniTask AnimateInAsync(string messageText, CancellationToken ct)
        {
            ...
            else if (isCommunityMessage)
            {
                ...
                currentSequence.Join(communityNameText.rectTransform.DOAnchorPos(communityTextFinalPosition, animationInDuration)).SetEase(backgroundEaseAnimationCurve).Join(communityNameText.DOColor(communityNameColor, animationInDuration)).SetEase(backgroundEaseAnimationCurve).Join(DOTween.To(() => communityNameTextBackgroundRenderer.size, v => communityNameTextBackgroundRenderer.size = v, textBackgroundTargetSize, animationInDuration).SetEase(backgroundEaseAnimationCurve)).Join(communityNameTextBackground.DOAnchorPos(communityTextFinalPosition, animationInDuration)).SetEase(backgroundEaseAnimationCurve).Join(communityNameTextBackgroundRenderer.DOColor(communityNameBackgroundColor, animationInDuration)).SetEase(backgroundEaseAnimationCurve);
            }
            ...
            currentSequence.Join(usernameText.rectTransform.DOAnchorPos(usernameFinalPosition, animationInDuration).SetEase(backgroundEaseAnimationCurve))
                           .Join(messageContent.rectTransform.DOAnchorPos(messageContentAnchoredPosition, animationInDuration).SetEase(backgroundEaseAnimationCurve))
                           .Join(messageContent.DOColor(currentSpritesColor, animationInDuration));
            ...
            isSpeakingIconFinalPosition = CalculateTalkingIconFinalPosition(usernameFinalPosition.x, cachedUsernameWidth, verifiedIconWidth, isSpeakingIconWidth, privateMessageIconWidth, isClaimedName, isPrivateMessage);
        ...
        private async UniTask AnimateOutAsync(CancellationToken ct)
        {
            ...
            else if (isCommunityMessage)
            {
                currentSequence.Join(communityNameText.rectTransform.DOAnchorPos(NametagViewConstants.ZERO_VECTOR, animationOutDurationHalf).SetEase(NametagViewConstants.LINEAR_EASE))
                               ...
                               .SetEase(backgroundEaseAnimationCurve)
                               .Join(communityNameTextBackground.DOAnchorPos(NametagViewConstants.ZERO_VECTOR, animationOutDurationTenth))
                               .SetEase(backgroundEaseAnimationCurve)
                               .Join(communityNameTextBackgroundRenderer.DOColor(NametagViewConstants.DEFAULT_TRANSPARENT_COLOR, animationOutDurationTenth))
                               .SetEase(backgroundEaseAnimationCurve);
            }
        ...
        [BurstCompile]
        private static float2 CalculatePreferredSize(float2 preferredSize, float usernameWidth, float nametagMarginWidth, float verifiedIconWidth, float isTalkingIconWidth,
            bool isTalking, float messageWidth, float maxWidth,
            float additionalHeight, float preferredHeight, bool isClaimedName, bool hasPrivateMessageIcon, float privateMessageTextWidth,
            float privateMessageIconWidth, out float availableWidthForPrivateMessage)
        {
        ...
        private void SetPrivateMessageText(string recipientValidatedName, string recipientWalletId, Color recipientNameColor)
        {
            ...
            currentRecipientNameColor = recipientNameColor;
        ...
        private static float2 CalculateVerifiedIconPosition(
            ...
            bool isTalking) =>
            new (
                usernamePositionX + (usernameWidth / 2) + (verifiedIconWidth / 2),
                0
            );
        ...
        private static float2 CalculateTalkingIconPosition(
            ...
            bool isClaimedName,
            float nametagMarginHeight) =>
            new (usernamePositionX + (usernameWidth / 2) + (isClaimedName ? verifiedIconWidth : 0) + speakingIconSpacing, 0);
        ...
        private static float2 CalculateUsernamePosition(float usernamePositionX, float verifiedIconWidth, float isTalkingIconWidth, bool isTalking) =>
            new (usernamePositionX - (verifiedIconWidth / 2), 0);
