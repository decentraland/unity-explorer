...
namespace StylizedGrass
{
    public static class ColorMapRendering
    {
        ...
        //Get all terrains
        private static readonly List<Terrain> terrains = new ();

        //Scene lighting state
        private static List<Light> dirLights;
        private static readonly Dictionary<Light, bool> originalLightStates = new ();
        ...
        private static readonly Dictionary<GameObject, TerrainState> originalTerrainStates = new ();
...
#region Rendering Setup
        public static void Render(GrassColorMapRenderer renderer)
        {
            ...
            if (renderer.colorMap.bounds.size == Vector3.zero) { renderer.colorMap.bounds = GetTerrainBounds(renderer.terrainObjects); }
        ...
        private static void SetupRenderer(GrassColorMapRenderer renderer)
        {
            ...
            else { Debug.LogError("[StylizedGrassRenderer] No Universal Render Pipeline is currently active."); }
#endif
        }
        ...
        private static void GenerateScalemap(List<Terrain> terrains, GrassColorMapRenderer renderer, RenderTexture rgb)
        {
            ...
            if (renderer.layerScaleSettings.Count > 0)
            {
                ...
                var alphaBuffer = new RenderTexture(renderer.resolution, renderer.resolution, 8, RenderTextureFormat.R8, RenderTextureReadWrite.Linear);
                var scalemapBuffer = new RenderTexture(renderer.resolution, renderer.resolution, 8, RenderTextureFormat.R8, RenderTextureReadWrite.Linear);

                //Alpha-weights combined makes up the scale map
                var scalemap = new RenderTexture(renderer.resolution, renderer.resolution, 8, RenderTextureFormat.R8, RenderTextureReadWrite.Linear);
                ...
                //Restore
                foreach (Terrain t in terrains) { t.SetSplatMaterialPropertyBlock(null); }

                //Add heightmap to alpha channel of rgb map
                Shader.SetGlobalTexture("_InputColormap", rgb);
                Shader.SetGlobalTexture("_InputHeightmap", scalemap);

                var colorBuffer = new RenderTexture(rgb);
            ...
            else { renderer.colorMap.hasScalemap = false; }
        }
...
#region Rendering Post
        private static void RestoreLighting(GrassColorMapRenderer renderer)
        {
            ...
            {
                if (originalLightStates.TryGetValue(light, out bool state)) { light.enabled = state; }
            }
        ...
        private static void RestoreTerrains(GrassColorMapRenderer renderer)
        {
            foreach (GameObject item in renderer.terrainObjects)
            {
                ...
                originalTerrainStates.TryGetValue(item, out TerrainState state);
...
#region Saving
        private static void Save(GrassColorMapRenderer renderer, GrassColorMap colorMap)
        {
            if (CopyBuffer)
            {
                ...
                if (EditorUtility.IsPersistent(colorMap))
                {
                    ...
                    var existingFile = (GrassColorMap)AssetDatabase.LoadAssetAtPath(colorMapAssetPath, typeof(GrassColorMap));
                ...
                if (colorMap.overrideTexture == false)
                {
                    ...
                    if (EditorUtility.IsPersistent(colorMap)) { SaveTextureData(renderer, colorMap); }
                    else
                    {
            ...
            else { colorMap.texture = renderTarget; }
        }
        ...
        private static void SaveTextureData(GrassColorMapRenderer renderer, GrassColorMap colorMap)
        {
            ...
            if (!bakedTexture) { bakedTexture = new Texture2D(colorMap.texture.width, colorMap.texture.height, TextureFormat.ARGB32, false, true); }

            byte[] bytes = bakedTexture.EncodeToPNG();
            File.WriteAllBytes(textureAssetPath, bytes);
...
#region Utilities
        ...
        public static double GetTexelSize(float texelSize, float worldSize) =>
            Math.Round(texelSize / worldSize, 2);
        ...
        public static int DetailPercentageToMipLevel(float percentage) =>
            Mathf.FloorToInt((100f - percentage) / 10);
