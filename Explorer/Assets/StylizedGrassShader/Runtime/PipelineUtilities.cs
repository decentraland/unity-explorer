...
namespace StylizedGrass
{
    public static class PipelineUtilities
    {
        ...
        public static void ValidatePipelineRenderers(ScriptableRendererData pass)
        {
            ...
            var m_rendererDataList = (ScriptableRendererData[])typeof(UniversalRenderPipelineAsset).GetField(renderDataListFieldName, bindings).GetValue(UniversalRenderPipeline.asset);
            var isPresent = false;

            for (var i = 0; i < m_rendererDataList.Length; i++)
            {
                if (m_rendererDataList[i] == pass) isPresent = true;
            }

            if (!isPresent) { AddRendererToPipeline(pass); }
            else
            {
#if SGS_DEV
        ...
        public static UniversalRendererData CreateEmptyRenderer(string name = "")
        {
            ...
            var r = (UniversalRendererData)defaultRenderer;
        ...
        public static void AddRenderFeature<T>(ScriptableRendererData forwardRenderer = null, bool persistent = true)
        {
            ...
            var feature = (ScriptableRendererFeature)ScriptableRendererFeature.CreateInstance(typeof(T).ToString());
            ...
            var m_RendererFeatures = (List<ScriptableRendererFeature>)renderFeaturesInfo.GetValue(forwardRenderer);
        ...
        public static void AssignRendererToCamera(UniversalAdditionalCameraData camData, ScriptableRendererData pass)
        {
            if (UniversalRenderPipeline.asset)
            {
                if (pass)
                {
                    ...
                    var rendererDataList = (ScriptableRendererData[])typeof(UniversalRenderPipelineAsset).GetField(renderDataListFieldName, BindingFlags.NonPublic | BindingFlags.Instance).GetValue(UniversalRenderPipeline.asset);
