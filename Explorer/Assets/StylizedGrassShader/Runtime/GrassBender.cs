...
#pragma warning restore 108,114
        ...
        private void SetupMaterial()
        {
            ...
            Material targetMat = GetMaterial(renderer.GetType());
        ...
        public void UpdateProperties()
        {
            ...
            if (SRPBatcherEnabled())
            {
                ...
                material.SetFloat(_SrcFactor, alphaBlending ? (int)BlendMode.SrcAlpha : (int)BlendMode.One);
                material.SetFloat(_DstFactor, alphaBlending ? (int)BlendMode.OneMinusSrcAlpha : (int)BlendMode.Zero);
        ...
        public void GetRenderer()
        {
            ...
            if (type == typeof(MeshRenderer)) { meshFilter = renderer.gameObject.GetComponent<MeshFilter>(); }
            else { meshFilter = null; }

            if (type == typeof(TrailRenderer)) { trailRenderer = renderer.gameObject.GetComponent<TrailRenderer>(); }
            else { trailRenderer = null; }

            if (type == typeof(LineRenderer)) { lineRenderer = renderer.gameObject.GetComponent<LineRenderer>(); }
            else { lineRenderer = null; }
        }
        ...
        private static readonly Vector3 TrailRotation = new (-90f, 0f, 0f);
        ...
        private void Update()
        {
            ...
            if (trailRenderer)
            {
                ...
                transform.rotation = Quaternion.Euler(TrailRotation);
            }
        }

        private static Material GetMaterial(Type type) =>
            type == typeof(MeshRenderer) || type == typeof(ParticleSystemRenderer) ? MeshMaterial : TrailMaterial;
