...
namespace StylizedGrass
{
    ...
    public class StylizedGrassRenderer : MonoBehaviour
    {
        ...
        public void OnEnable()
        {
            ...
            if (colorMap) { colorMap.SetActive(); }
            else
            {
        ...
        private void UpdateWind()
        {
            if (listenToWindZone && windZone && windZone.gameObject.activeInHierarchy)
            {
                ...
                timeOffset += deltaTime * m_windMain * Time.timeScale;
        ...
        private void DrawDebugGUI(bool sceneView)
        {
            vectorRT = (RenderTexture)Shader.GetGlobalTexture("_GrassOffsetVectors");

            if (!vectorRT) return;

            var imgRect = new Rect(5, 5, 256, 256);

            //Set to UI debug image
            if (debug && !sceneView) { GUI.DrawTexture(imgRect, vectorRT); }
