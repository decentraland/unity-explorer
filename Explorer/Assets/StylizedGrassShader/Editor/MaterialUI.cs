...
namespace StylizedGrass
{
    ...
    public class MaterialUI : ShaderGUI
    {
        ...
        private StylizedGrassGUI.Material.Section renderingSection;
        private StylizedGrassGUI.Material.Section mapsSection;
        private StylizedGrassGUI.Material.Section colorSection;
        private StylizedGrassGUI.Material.Section shadingSection;
        private StylizedGrassGUI.Material.Section verticesSection;
        private StylizedGrassGUI.Material.Section windSection;
        ...
        private void OnEnable(MaterialEditor materialEditorIn)
        {
            ...
            foreach (Object obj in materialEditorIn.targets) { MaterialChanged((Material)obj); }
        }
        ...
        private void SetMaterialKeywords(Material material)
        {
            ...
            //Keywords
            var lightingMode = (int)material.GetFloat("_LightingMode");
        ...
        private void DrawHeader()
        {
            ...
            var c = new GUIContent($"{AssetInfo.ASSET_NAME} v{AssetInfo.INSTALLED_VERSION}");
        ...
        {
            renderingSection.Expanded = StylizedGrassGUI.Material.DrawHeader(renderingSection.title, renderingSection.Expanded, () => SwitchSection(renderingSection));
            ...
            if (EditorGUILayout.BeginFadeGroup(renderingSection.anim.faded))
            {
                ...
                using (new EditorGUI.DisabledGroupScope(_LightingMode.floatValue == 0)) { materialEditor.ShaderProperty(_ReceiveShadows, new GUIContent("Receive shadows", "Apply shadows cast by other objects onto this material.\n\nShadow casting behaviour can be set on a per Mesh Renderer basis.")); }
        ...
        {
            mapsSection.Expanded = StylizedGrassGUI.Material.DrawHeader(mapsSection.title, mapsSection.Expanded, () => SwitchSection(mapsSection));
        ...
        {
            colorSection.Expanded = StylizedGrassGUI.Material.DrawHeader(colorSection.title, colorSection.Expanded, () => SwitchSection(colorSection));
        ...
        {
            shadingSection.Expanded = StylizedGrassGUI.Material.DrawHeader(shadingSection.title, shadingSection.Expanded, () => SwitchSection(shadingSection));
...
#if UNITY_2022_1_OR_NEWER
                    Texture customReflectionUsed = RenderSettings.customReflectionTexture;
#else
                    var customReflectionUsed = RenderSettings.customReflection;
#endif
                    if (_EnvironmentReflections.floatValue == 1f && RenderSettings.defaultReflectionMode == DefaultReflectionMode.Custom && !customReflectionUsed) { EditorGUILayout.HelpBox("Environment reflection source is set to \"Custom\" but no cubemap is assigned. ", MessageType.Warning); }
        ...
        {
            verticesSection.Expanded = StylizedGrassGUI.Material.DrawHeader(verticesSection.title, verticesSection.Expanded, () => SwitchSection(verticesSection));
        ...
        {
            windSection.Expanded = StylizedGrassGUI.Material.DrawHeader(windSection.title, windSection.Expanded, () => SwitchSection(windSection));
            ...
            if (EditorGUILayout.BeginFadeGroup(windSection.anim.faded))
            {
                ...
                if (StylizedGrassRenderer.Instance && StylizedGrassRenderer.Instance.windZone && StylizedGrassRenderer.Instance.listenToWindZone) { StylizedGrassGUI.DrawActionBox("  Wind strength and speed are influenced by a Wind Zone component", "Select Wind Zone", MessageType.Info, () => Selection.activeObject = StylizedGrassRenderer.Instance.windZone); }
        ...
        private void SwitchSection(StylizedGrassGUI.Material.Section s)
        {
            /*
    ...
    public class MinMaxSlider : MaterialPropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
        {
            ...
            if (EditorGUI.EndChangeCheck()) { prop.vectorValue = new Vector4(minVal, maxVal, 0f, 0f); }
