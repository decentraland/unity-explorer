...
namespace StylizedGrass
{
    ...
    {
        private GrassColorMapRenderer script;
        private SerializedProperty colorMap;
        private SerializedProperty resIdx;
        private SerializedProperty resolution;
        private SerializedProperty cullingMask;
        private SerializedProperty textureDetail;
        private SerializedProperty useOriginalMaterials;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (colorMap.objectReferenceValue)
            {
                ...
                {
                    var saveColorMap = new Action(SaveColorMap);
                ...
                if (script.colorMap.overrideTexture) { EditorGUILayout.HelpBox("The assigned color map uses a texture override. Rendering a new/updated color map will revert this.", MessageType.Warning); }
            }
            ...
            if (EditorGUI.EndChangeCheck()) { EditorApplication.delayCall += Validate; }
            ...
            else
            {
                ...
                if (expandRendering)
                {
                    ...
                    if (useOriginalMaterials.boolValue == false)
                    {
                        ...
                        if (hasMeshRenderers) { EditorGUILayout.HelpBox("One or more terrain objects is a Mesh Renderer, this option must be enabled to correctly capture their albedo color.", MessageType.Warning); }
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        resIdx.intValue = EditorGUILayout.Popup("Resolution", resIdx.intValue,
                            new[] { "64x64", "128x128", "256x256", "512x512", "1024x1024", "2048x2048", "4096x4096" },
                            GUILayout.MaxWidth(EditorGUIUtility.labelWidth + 100f));

                        if (EditorGUI.EndChangeCheck()) { resolution.intValue = ColorMapRendering.IndexToResolution(resIdx.intValue); }
                    }
                ...
                if (colorMap.objectReferenceValue && Application.isPlaying) { EditorGUILayout.HelpBox("[Play mode] To improve runtime baking performance, no rendering data will be saved to disk. Instead the render result will be kept in memory and sent directly to the shader.", MessageType.Info); }

                if (GUILayout.Button(new GUIContent("  Render", EditorGUIUtility.IconContent(iconPrefix + "Animation.Record").image), GUILayout.Height(30f))) { script.Render(); }
        ...
        private void DrawLayerHeightSettings()
        {
            ...
            if (script.layerScaleSettings != null && script.layerScaleSettings.Count > 0)
            {
                using (new EditorGUILayout.VerticalScope())
                {
                    ...
                    for (var i = 0; i < script.layerScaleSettings.Count; i++)
                    {
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            if (GUILayout.Button(new GUIContent("", EditorGUIUtility.IconContent(iconPrefix + "TreeEditor.Trash").image, "Delete item"))) { script.layerScaleSettings.RemoveAt(i); }
                        }
                    }
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        if (colorMap.objectReferenceValue)
                        {
                            ...
                            if (EditorUtility.IsPersistent(script.colorMap) == false) { script.Render(); }
                        }
                    }
                }
            }
            else { EditorGUILayout.HelpBox("No scale settings for terrain layers, grass will stay a uniform scale", MessageType.None); }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(new GUIContent("Add layer setting", EditorGUIUtility.IconContent(iconPrefix + "Toolbar Plus").image)))
                {
                    ...
                    for (var i = 0; i < terrainLayers.Length; i++)
                    {
                        ...
                        if (script.layerScaleSettings.Find(x => x.layerIndex == i) == null) { menu.AddItem(new GUIContent(terrainLayers[i].name), false, AddTerrainLayerMask, i); }
                        else { menu.AddDisabledItem(new GUIContent(terrainLayers[i].name), true); }
                    }
        ...
        {
            var s = new GrassColorMapRenderer.LayerScaleSettings();
            s.layerIndex = (int)id;
        ...
        private void RefreshTerrainLayers()
        {
            ...
            for (var i = 0; i < layerNames.Length; i++) { layerNames[i] = new GUIContent(t.terrainData.terrainLayers[i] ? t.terrainData.terrainLayers[i].name : "(Missing)"); }
        }

        public override bool HasPreviewGUI() =>
            script.colorMap && script.colorMap.texture;
        ...
        public override void OnPreviewGUI(Rect r, GUIStyle background)
        {
            ...
            if (previewColor) { GUI.DrawTexture(r, script.colorMap.texture, ScaleMode.ScaleToFit, false); }
            else { EditorGUI.DrawTextureAlpha(r, script.colorMap.texture, ScaleMode.ScaleToFit); }
