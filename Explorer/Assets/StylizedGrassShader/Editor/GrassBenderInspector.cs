...
namespace StylizedGrass
{
    ...
    {
        private GrassBender bender;
        private Vector3[] points;
        private Vector3[] circlePoints;

        private SerializedProperty renderer;
        private SerializedProperty sortingLayer;
        private SerializedProperty flattenStrength;
        private SerializedProperty heightOffset;
        private SerializedProperty scaleMultiplier;
        private SerializedProperty pushStrength;

        //Mesh
        private SerializedProperty alphaBlending;

        //Trail
        private SerializedProperty forceTrailUpdating;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (renderer.objectReferenceValue)
            {
                ...
                if (renderer.objectReferenceValue.GetType() == typeof(MeshRenderer)) { EditorGUILayout.PropertyField(scaleMultiplier); }

                if (renderer.objectReferenceValue.GetType() == typeof(TrailRenderer)) { EditorGUILayout.PropertyField(forceTrailUpdating); }
                ...
                if (GrassBender.SRPBatcherEnabled() == false) { EditorGUILayout.HelpBox("These options are only available if the SRP Batcher is enabled", MessageType.Info); }
                ...
                if (isTrailOrLine) { EditorGUI.indentLevel--; }
            }
            else
            {
        ...
        public void DrawDottedLines(Vector3[] lineSegments, float screenSpaceSize)
        {
            ...
            float dashSize = screenSpaceSize * EditorGUIUtility.pixelsPerPoint;

            for (var i = 0; i < lineSegments.Length - 1; i += 2)
            {
                Vector3 p1 = lineSegments[i + 0];
                Vector3 p2 = lineSegments[i + 1];

                if (p1 == null || p2 == null) continue;

                Handles.color = new Color(1, 1, 1, bender.trailRenderer.colorGradient.Evaluate(1f - (i / (float)(lineSegments.Length - 1))).r);
                Handles.DrawAAPolyLine(Texture2D.whiteTexture, dashSize, p1, p2);
            }
