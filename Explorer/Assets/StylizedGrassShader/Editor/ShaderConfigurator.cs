...
namespace StylizedGrass
{
    public class ShaderConfigurator
    {
        ...
        {
            get => (Integration)EditorPrefs.GetInt(PlayerSettings.productName + "_SGS_SHADER_INTEGRATION", 0);
        ...
        public static void SetIntegration(Integration integration)
        {
            ...
            Debug.Log("Shader file modified to use " + integration + " integration");
        ...
        private static void ToggleCodeBlock(string filePath, string id, bool enable)
        {
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                if (lines[i].StartsWith("/* Integration: ") && enable) { lines[i] = $"/* Integration: {id} */"; }
            ...
            foreach (CodeBlock codeBlock in codeBlocks)
            {
                ...
                for (int i = codeBlock.startLine + 1; i < codeBlock.endLine; i++)
                {
                    ...
                    if (enable)
                    {
                        if (lines[i].StartsWith("//")) lines[i] = lines[i].Remove(0, 2);
                    }
        ...
        private static void SetIncludePath(Integration config)
        {
            ...
            switch (config)
            {
                case Integration.VegetationStudio:
                    GUID = VegetationStudioGUID;
                    break;
                case Integration.NatureRendererLegacy:
                    GUID = NatureRendererLegacyGUID;
                    break;
                case Integration.NatureRenderer:
                    GUID = NatureRendererGUID;
                    break;
                case Integration.GPUInstancer:
                    GUID = GPUInstancerGUID;
                    break;
                case Integration.FoliageRenderer:
                    GUID = FoliageRendererGUID;
                    break;
                default:
                    GUID = string.Empty;
                    break;
            ...
            if (libraryFilePath == string.Empty)
            {
                if (EditorUtility.DisplayDialog(AssetInfo.ASSET_NAME,
                        ...
                        "This means it was changed by the author, you deleted the \".meta\" file at some point, or the asset simply isn't installed.", "Revert back", "Continue with errors")) { SetIntegration(Integration.None); }
            }
            else { SetIncludePath(ShaderFilePath, config.ToString(), libraryFilePath); }
        }
    ...
    internal class KeywordStripper : IPreprocessShaders
    {
        ...
        public void OnProcessShader(Shader shader, ShaderSnippetData snippet, IList<ShaderCompilerData> compilerDataList)
        {
            ...
            if (StripUnusedPasses(shader, snippet)) { compilerDataList.Clear(); }
            ...
            for (int i = compilerDataList.Count - 1; i >= 0; i--)
            {
                ...
                if (removeInput) { compilerDataList.RemoveAt(i); }
            }
        ...
        {
            if (StripUnusedPasses(shader, snippet)) { return true; }

            foreach (ShaderKeyword keyword in excludedKeywords)
            {
                if (StripKeyword(shader, compilerData, keyword, snippet)) { return true; }
            }
