...
#region Initialization
        ...
        private void Awake()
        {
            ...
            foreach (Transform child in ActionButtonParent) { Destroy(child.gameObject); }
        ...
        private bool WalletConnected()
        {
            ...
            if (!isConnected) { LogPlayground("Please authenticate to connect a wallet first."); }
...
#region Wallet Connection
        ...
        {
            var walletOptions = new WalletOptions(provider: WalletProvider.InAppWallet, chainId: ChainId, new InAppWalletOptions(authprovider: AuthProvider.Guest));
        ...
        {
            if (!Enum.TryParse<AuthProvider>(Social.ToString(), out AuthProvider parsedOAuthProvider)) { parsedOAuthProvider = AuthProvider.Google; }

            var walletOptions = new WalletOptions(provider: WalletProvider.InAppWallet, chainId: ChainId, new InAppWalletOptions(authprovider: parsedOAuthProvider));
            IThirdwebWallet wallet = await ThirdwebManager.Instance.ConnectWallet(walletOptions);

            if (AlwaysUpgradeToSmartWallet) { wallet = await ThirdwebManager.Instance.UpgradeToSmartWallet(wallet, chainId: ChainId, smartWalletOptions: new SmartWalletOptions(sponsorGas: true)); }
        ...
        private async void Wallet_Email()
        {
            ...
            var walletOptions = new WalletOptions(provider: WalletProvider.InAppWallet, chainId: ChainId, new InAppWalletOptions(email: Email));
            IThirdwebWallet wallet = await ThirdwebManager.Instance.ConnectWallet(walletOptions);
        ...
        private async void Wallet_Phone()
        {
            ...
            var walletOptions = new WalletOptions(provider: WalletProvider.InAppWallet, chainId: ChainId, new InAppWalletOptions(phoneNumber: Phone));
            IThirdwebWallet wallet = await ThirdwebManager.Instance.ConnectWallet(walletOptions);
...
#region Wallet Actions
        ...
        private async void WalletAction_Transfer()
        {
            ...
            ThirdwebTransactionReceipt receipt = await ThirdwebManager.Instance.ActiveWallet.Transfer(chainId: ChainId, toAddress: await ThirdwebManager.Instance.ActiveWallet.GetAddress(), weiAmount: 0);
...
#region Contract Interaction
        private async void Contract_Read()
        {
            ...
            ThirdwebContract contract = await ThirdwebManager.Instance.GetContract(address: usdcContractAddressArbitrum, chainId: 42161);
            BigInteger result = await contract.ERC20_BalanceOf(ownerAddress: await Utils.GetAddressFromENS(ThirdwebManager.Instance.Client, "vitalik.eth"));
        ...
        private async void Contract_Write()
        {
            ...
            ThirdwebContract contract = await ThirdwebManager.Instance.GetContract(address: "0x3EE304A2cBf24F73510C6C590cFcd10bEd0E6F70", chainId: ChainId);
        ...
        {
            ThirdwebContract contract = await ThirdwebManager.Instance.GetContract(address: "0xBD0334AC7FADA28CcD27Fa09838e9EA4c39117Db", chainId: ChainId);
        ...
        private async void Contract_WriteCustom()
        {
            ...
            ThirdwebTransactionReceipt transactionReceipt = await contract.Write(wallet: ThirdwebManager.Instance.ActiveWallet, method: method, weiValue: 0, parameters: new object[] { newOwner });
