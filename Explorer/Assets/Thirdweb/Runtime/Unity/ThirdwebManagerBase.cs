...
namespace Thirdweb.Unity
{
    ...
    public class ReownOptions
    {
        ...
        {
            if (singleWalletId != null && (includedWalletIds != null || excludedWalletIds != null || featuredWalletIds != null)) { throw new ArgumentException("singleWalletId cannot be used with includedWalletIds, excludedWalletIds, or featuredWalletIds."); }
    ...
    public abstract class ThirdwebManagerBase : MonoBehaviour
    {
        ...
        {
            if (!Initialized) { throw new InvalidOperationException("ThirdwebManager is not initialized."); }
        ...
        {
            if (walletOptions == null) { throw new ArgumentNullException(nameof(walletOptions)); }

            if (walletOptions.ChainId <= 0) { throw new ArgumentException("ChainId must be greater than 0."); }
            ...
            switch (walletOptions.Provider)
            {
                ...
                case WalletProvider.EcosystemWallet:
                    if (walletOptions.EcosystemWalletOptions == null) { throw new ArgumentException("EcosystemWalletOptions must be provided for EcosystemWallet provider."); }

                    if (string.IsNullOrEmpty(walletOptions.EcosystemWalletOptions.EcosystemId)) { throw new ArgumentException("EcosystemId must be provided for EcosystemWallet provider."); }
        ...
        public virtual async Task<SmartWallet> UpgradeToSmartWallet(IThirdwebWallet personalWallet, BigInteger chainId, SmartWalletOptions smartWalletOptions)
        {
            ...
            if (smartWalletOptions == null) { throw new ArgumentNullException(nameof(smartWalletOptions)); }

            if (chainId <= 0) { throw new ArgumentException("ChainId must be greater than 0."); }
