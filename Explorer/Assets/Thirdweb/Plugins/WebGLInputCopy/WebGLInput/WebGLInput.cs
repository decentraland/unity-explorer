...
namespace WebGLSupport
{
    ...
    public class WebGLInput : MonoBehaviour, IComparable<WebGLInput>
    {
        ...
        private static readonly Dictionary<int, WebGLInput> instances = new ();
        ...
        private void Awake()
        {
            ...
            {
                if (input.EnableMobileSupport) { gameObject.AddComponent<WebGLInputMobile>(); }
                else
                {
        ...
        {
            Rect rect = input.GetScreenCoordinates();
            ...
            else
            {
                ...
                return new RectInt(x, y, (int)rect.width, 1);
            }
        }
        ...
        public void OnSelect()
        {
            ...
            RectInt rect = GetElemetRect();
            bool isPassword = input.contentType == ContentType.Password;

            int fontSize = Mathf.Max(14, input.fontSize); // limit font size : 14 !!
...
#endif
            if (input.OnFocusSelectAll) { WebGLInputPlugin.WebGLInputSetSelectionRange(id, 0, input.text.Length); }
            else { WebGLInputPlugin.WebGLInputSetSelectionRange(id, input.caretPosition, input.caretPosition); }
        ...
        public void SyncText(int? cursorIndex = null)
        {
            ...
            WebGLInput instance = instances[id];

            WebGLInputPlugin.WebGLInputText(id, instance.input.text);

            if (cursorIndex.HasValue) { WebGLInputPlugin.WebGLInputSetSelectionRange(id, cursorIndex.Value, cursorIndex.Value); }
        }
        ...
        private static IEnumerator Blur(int id)
        {
            ...
            bool block = instances[id].blurBlock; // get blur block state
        ...
        private static void OnValueChange(int id, string value)
        {
            ...
            if (!instance.input.ReadOnly) { instance.input.text = value; }

            // InputField.ContentType.Name が Name の場合、先頭文字が強制的大文字になるため小文字にして比べる
            if (instance.input.contentType == ContentType.Name)
            {
                if (string.Compare(instance.input.text, value, true) == 0) { value = instance.input.text; }
            }

            // InputField の ContentType による整形したテキストを HTML の input に再設定します
            if (value != instance.input.text)
            {
                int start = WebGLInputPlugin.WebGLInputSelectionStart(id);
                int end = WebGLInputPlugin.WebGLInputSelectionEnd(id);

                // take the offset.when char remove from input.
                int offset = instance.input.text.Length - value.Length;
        ...
        {
            if (!instances[id].input.ReadOnly) { instances[id].input.text = value; }
        }
        ...
        private static void OnKeyboardEvent(int id, int mode, string key, int code, int shiftKey,
            int ctrlKey, int altKey)
        {
            if (!instances.ContainsKey(id))
                return;
            ...
            KeyboardEventHandler cb = mode switch
                                      {
                                          1 => OnKeyboardDown,
                                          2 => OnKeyboardUp,
                                          _ => default(KeyboardEventHandler),
                                      };

            if (cb != null) { cb(instance, new KeyboardEvent(key, code, shiftKey != 0, ctrlKey != 0, altKey != 0)); }
        }
        ...
        {
            Rect a = input.GetScreenCoordinates();
            Rect b = other.input.GetScreenCoordinates();
            int res = b.y.CompareTo(a.y);
        ...
        {
            private static readonly List<WebGLInput> inputs = new ();
