...
namespace Mediapipe.Unity.Sample.PoseLandmarkDetection
{
  public class PoseLandmarkerRunner : VisionTaskApiRunner<PoseLandmarker>
  {
    ...
    private void OnPoseLandmarkDetectionOutput(PoseLandmarkerResult result, Image image, long timestamp)
    {
      ...
      if(result.poseLandmarks is { Count: > 0 })
      {
          for (var i = 0; i < result.poseWorldLandmarks.Count; i++)
          {
              Landmarks landmark = result.poseWorldLandmarks[i];

              for (var j = 11; j < 22; j++)
              {
                  Tasks.Components.Containers.Landmark landmark1 = landmark.landmarks[j];

                  resultPos[j] = new Vector3(landmark1.x, -landmark1.y, landmark1.z);

                  // resultPos[j]
                  // Debug.Log($"DRAW LATER {i},{j} | {landmark1.x},{landmark1.y},{landmark1.z}");
              }
          }
      }      DisposeAllMasks(result);
    }
