...
namespace Microsoft.ClearScript.Util
{
    internal static class MiscHelpers
    {
        ...
        public static Type GetCOMType(Guid clsid, string serverName)
        {
            if (!TryGetCOMType(clsid, serverName, out var type))
            {
                throw new TypeLoadException(FormatInvariant("Could not find a registered class for '{0}'",
                    clsid.ToString("B")));
            }
        ...
        {
            type = Guid.TryParseExact(progID, "B", out var clsid)
        ...
        public static bool IsDispIDName(this string name, out int dispid)
        {
            ...
            if (match.Success && int.TryParse(match.Groups[1].Value, NumberStyles.AllowLeadingSign,
                    CultureInfo.InvariantCulture, out dispid))
            {
                return true;
            }
        ...
        public static string FormatCode(string code)
        {
            ...
            lines = lines.SkipWhile(string.IsNullOrWhiteSpace).Reverse().SkipWhile(string.IsNullOrWhiteSpace).Reverse()
                .ToArray();
            if (lines.Length > 0)
            {
                     ...
                     indentLength--)
                {
                    ...
                    if (lines.Skip(1).All(line =>
                            string.IsNullOrWhiteSpace(line) || line.StartsWith(indent, StringComparison.Ordinal)))
                    {
                        lines = lines.Select(line =>
                            string.IsNullOrWhiteSpace(line) ? string.Empty : line.Substring(indent.Length)).ToArray();
                        break;
                    }
                }
            }
        ...
        {
            return searchPath.Split(searchPathSeparators, StringSplitOptions.RemoveEmptyEntries)
                .Distinct(StringComparer.OrdinalIgnoreCase);
        }

        public static StringComparison GetMemberNameComparison(this BindingFlags bindFlags)
        {
            return bindFlags.HasFlag(BindingFlags.IgnoreCase)
        ...
        {
            var path = Path.Combine(basePath, "Microsoft", "ClearScript", ClearScriptVersion.Triad,
