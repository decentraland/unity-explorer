...
#region Observable Event
        ...
        private void SetHeaders()
        {
            ...
            {
                foreach (KeyValuePair<string, string> item in Options.ExtraHeaders) { _transport.AddHeader(item.Key, item.Value); }
            }
        }
                ...
                while (true)
                {
                    ...
                    catch (Exception e)
                    {
                        if (_expectedExceptions.Contains(e.GetType()))
                        {
                            ...
                            if (Attempts > 0) { OnReconnectError?.Invoke(this, e); }
        ...
        public async Task ConnectAsync()
        {
            ...
            while (_isConnectCoreRunning) { await Task.Delay(20); }

            if (_connectCoreException != null) { throw _connectCoreException; }
        }
        ...
        private void ConnectedHandler(ConnectedMessage msg)
        {
            ...
            if (Attempts > 0) { OnReconnected?.Invoke(this, Attempts); }
        ...
        private void EventMessageHandler(EventMessage m)
        {
            ...
            foreach (OnAnyHandler item in _onAnyHandlers)
            {
                try { item(m.Event, res); }
                catch (Exception e) { Debug.WriteLine(e); }
            }

            if (_eventHandlers.ContainsKey(m.Event))
            {
                try { _eventHandlers[m.Event](res); }
                catch (Exception e) { Debug.WriteLine(e); }
            }
        }
        ...
        private void AckMessageHandler(ClientAckMessage m)
        {
            if (_ackHandlers.ContainsKey(m.Id))
            {
                ...
                try { _ackHandlers[m.Id](res); }
                finally { _ackHandlers.Remove(m.Id); }
            }
        }
        ...
        public async Task DisconnectAsync()
        {
            if (Connected)
            {
                ...
                try { await _transport.SendAsync(msg, CancellationToken.None).ConfigureAwait(false); }
                catch (Exception e) { Debug.WriteLine(e); }
        ...
        private async void InvokeDisconnect(string reason)
        {
            if (Connected)
            {
                ...
                if (reason != DisconnectReason.IOServerDisconnect && reason != DisconnectReason.IOClientDisconnect)
                {
                    ...
                    if (Options.Reconnection) { ConnectCore(); }
                }
            }
        }
