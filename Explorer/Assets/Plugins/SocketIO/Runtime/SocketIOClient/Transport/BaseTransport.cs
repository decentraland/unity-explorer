...
namespace SocketIOClient.Transport
{
    public abstract class BaseTransport : IObserver<string>, IObserver<byte[]>, IObservable<IMessage>, IDisposable
    {
        ...
        protected virtual async Task OpenAsync(OpenedMessage msg)
        {
            ...
            if (Options.EIO == 3 && string.IsNullOrEmpty(Namespace)) { return; }
            ...
            if (Options.EIO == 4)
            {
                if (Options.Auth != null) { connectMsg.AuthJsonStr = JsonSerializer.Serialize(new[] { Options.Auth }).Json.TrimStart('[').TrimEnd(']'); }
            }
        ...
        public void OnNext(string text)
        {
            ...
            IMessage msg = MessageFactory.CreateMessage(Options.EIO, text);
            ...
            if (msg.Type == MessageType.Opened) { OpenAsync(msg as OpenedMessage).ConfigureAwait(false); }
        ...
        public void OnNext(byte[] bytes)
        {
            ...
            {
                IMessage msg = _messageQueue.Peek();
