...
namespace SocketIOClient.Transport
{
    public class HttpTransport : BaseTransport
    {
        ...
        private void StartPolling(CancellationToken cancellationToken)
        {
            Task.Factory.StartNew(async () =>
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    try { await _httpPollingHandler.GetAsync(_httpUri, CancellationToken.None).ConfigureAwait(false); }
                    catch (Exception e)
                    {
                        retry++;
        ...
        public override async Task ConnectAsync(Uri uri, CancellationToken cancellationToken)
        {
            ...
            if (_pollingTokenSource != null) { _pollingTokenSource.Cancel(); }
        ...
        public override async Task SendAsync(Payload payload, CancellationToken cancellationToken)
        {
            ...
            if (payload.Bytes != null && payload.Bytes.Count > 0) { await _httpPollingHandler.PostAsync(_httpUri, payload.Bytes, cancellationToken); }
        }
