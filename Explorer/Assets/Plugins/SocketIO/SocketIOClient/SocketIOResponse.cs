...
namespace SocketIOClient
{
    public class SocketIOResponse
    {
        ...
        {
            JsonElement element = GetValue(index);
            string json = element.GetRawText();
            return SocketIO.JsonSerializer.Deserialize<T>(json, InComingBytes);
        }

        public JsonElement GetValue(int index = 0) =>
            _array[index];
        ...
        public override string ToString()
        {
            ...
            foreach (JsonElement item in _array)
            {
                builder.Append(item.GetRawText());

                if (_array.IndexOf(item) < _array.Count - 1) { builder.Append(','); }
            }
