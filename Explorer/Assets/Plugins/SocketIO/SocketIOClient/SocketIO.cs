        ...
        private Uri ServerUri
        {
            ...
            set
            {
                if (_serverUri != value)
                {
                    ...
                    if (value != null && value.AbsolutePath != "/") { Namespace = value.AbsolutePath; }
                }
            }
        }
...
#region Socket.IO event
        ...
        private async Task InitTransportAsync()
        {
            ...
            {
                IHttpPollingHandler handler = HttpPollingHandler.CreateHandler(transportOptions.EIO, HttpClient);
                Transport = new HttpTransport(transportOptions, handler);
            }
            else
            {
                IClientWebSocket ws = ClientWebSocketProvider();
        ...
        private void ConnectInBackground(CancellationToken cancellationToken)
        {
            Task.Factory.StartNew(async () =>
            {
                while (true)
                {
                    ...
                    {
                        bool needBreak = await AttemptAsync(e);
                        if (needBreak) break;

                        bool canHandle = CanHandleException(e);
                        if (!canHandle) throw;
                    }
                }
            }, cancellationToken);
        ...
        private async Task<bool> AttemptAsync(Exception e)
        {
            ...
            {
                if (_reconnectionDelay < Options.ReconnectionDelayMax) { _reconnectionDelay += 2 * Options.RandomizationFactor; }
        ...
        private async Task<TransportProtocol> GetProtocolAsync()
        {
            ...
            if (Options.Transport == TransportProtocol.Polling && Options.AutoUpgrade)
            {
                ...
                try
                {
                    ...
                    if (text.Contains("websocket")) { return TransportProtocol.WebSocket; }
                }
                catch (Exception e)
                {
#if DEBUG
        ...
        public async Task ConnectAsync()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    if (_connCts.IsCancellationRequested) { break; }

                    if (_backgroundException != null) { throw new ConnectionException($"Cannot connect to server '{ServerUri}'", _backgroundException); }
        ...
        private void ConnectedHandler(ConnectedMessage msg)
        {
            ...
            if (_attempts > 0) { OnReconnected.TryInvoke(this, _attempts); }
        ...
        private void EventMessageHandler(EventMessage m)
        {
            ...
            foreach (OnAnyHandler item in _onAnyHandlers) { item.TryInvoke(m.Event, res); }

            if (_eventHandlers.ContainsKey(m.Event)) { _eventHandlers[m.Event].TryInvoke(res); }
        }
        ...
        private void BinaryMessageHandler(BinaryMessage msg)
        {
            ...
            foreach (OnAnyHandler item in _onAnyHandlers) { item.TryInvoke(msg.Event, response); }

            if (_eventHandlers.ContainsKey(msg.Event)) { _eventHandlers[msg.Event].TryInvoke(response); }
        }
        ...
        {
            if (_eventHandlers.ContainsKey(eventName)) { _eventHandlers.Remove(eventName); }
        ...
        {
            if (handler != null) { _onAnyHandlers.Add(handler); }
        }

        public void PrependAny(OnAnyHandler handler)
        {
            if (handler != null) { _onAnyHandlers.Insert(0, handler); }
        }

        public void OffAny(OnAnyHandler handler)
        {
            if (handler != null) { _onAnyHandlers.Remove(handler); }
        }
        ...
        internal async Task ClientAckAsync(int packetId, CancellationToken cancellationToken, params object[] data)
        {
            ...
            {
                JsonSerializeResult result = JsonSerializer.Serialize(data);
        ...
        private async Task InvokeDisconnect(string reason)
        {
            if (Connected)
            {
                ...
                if (reason != DisconnectReason.IOServerDisconnect && reason != DisconnectReason.IOClientDisconnect)
                {
                    ...
                    if (Options.Reconnection) { ConnectInBackground(); }
                }
            }
        }
        ...
        {
            if (!_expectedExceptions.Contains(type)) { _expectedExceptions.Add(type); }
        }
