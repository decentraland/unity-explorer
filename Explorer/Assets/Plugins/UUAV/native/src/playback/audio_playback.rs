...
const AUDIO_CONSUMER_TIMEOUT: Duration = Duration::from_millis(250);
...
impl AudioPlayback {
    ...
    pub(super) fn handle_packet(
        ...
        self.pump(start_offset, cancel, seek, transport, poll_controls)
    }
    ...
    pub(super) fn is_drained(&self) -> bool {
        self.ring.is_drained() || self.consumer_gone()
    }
            ...
            match self.decoder.receive()? {
                Decoded::Frame(frame) => {
                    ...
                    loop {
                        ...
                        if self.note_refusal(transport) {
                            break;
                        }
    ...
    fn note_refusal(&mut self, transport: &AtomicTransport) -> bool {
        if !self.blocks_video || !transport.is_playing() {
            self.refused_since = None;
            return false;
        }
        if self.refused_since.is_none() {
            self.refused_since = Some(Instant::now());
        }
        self.consumer_gone()
    }
    ...
    fn consumer_gone(&self) -> bool {
        self.refused_since
            .is_some_and(|since| since.elapsed() >= AUDIO_CONSUMER_TIMEOUT)
    }
}
