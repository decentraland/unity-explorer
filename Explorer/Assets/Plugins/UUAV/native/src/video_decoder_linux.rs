...
/// A decoded VAAPI frame. The `VASurfaceID` behind it stays alive (and
/// thus valid for export) for as long as `frame` is held: AVFrame
/// refcounting owns the surface through its hwframes pool, so handing
/// frames from the playback thread to the render thread needs no extra
/// retain/release.
...
impl VideoFrame {
    /// The decoded `AVFrame` (`AV_PIX_FMT_VAAPI`), for
    /// `av_hwframe_map` into a DRM PRIME descriptor at present time.
    pub(crate) const fn av_frame(&self) -> *const ff::AVFrame {
        self.frame.as_ptr()
    }
...
/// VAAPI hardware video decoder. Hardware decoding is mandatory: the
/// stream is rejected if the codec offers no VAAPI path or the decoded
/// surfaces are not NV12.
...
impl VideoDecoder {
    ...
    pub(crate) fn new(stream: Stream, hw: &HwDeviceContext) -> Result<Self> {
        ...
        unsafe {
            ...
            if (*raw).hw_device_ctx.is_null() {
                return Err(anyhow!("failed to reference the VAAPI device context"));
            }
            (*raw).get_format = Some(require_vaapi_format);
    ...
    pub(crate) fn receive(&mut self) -> Result<Decoded<VideoFrame>> {
        ...
        if frame.format() != ff::AVPixelFormat::AV_PIX_FMT_VAAPI as c_int {
            return Err(anyhow!(
                "decoder produced a non-VAAPI frame (format {}); hardware decoding is mandatory",
                frame.format()
            ));
        }

        // NV12-only, matching the policy of the D3D11/VideoToolbox
        // siblings; anything else (P010/10-bit, ...) is rejected
        let sw_format = frame.hw_frames_ctx().context("VAAPI frame")?.sw_format;
        if sw_format != ff::AVPixelFormat::AV_PIX_FMT_NV12 {
            return Err(anyhow!(
                "unsupported VAAPI surface format {sw_format:?}; only NV12 is supported"
            ));
        }
...
/// Rejects codecs without a VAAPI hardware path. Software-only decoders
/// never invoke `get_format`, so without this probe the rejection would
/// only happen after a software frame was already decoded.
fn ensure_vaapi_support(codec: *const ff::AVCodec) -> Result<()> {
    for index in 0.. {
        ...
        if by_device_ctx
            && config.device_type == ff::AVHWDeviceType::AV_HWDEVICE_TYPE_VAAPI
            && config.pix_fmt == ff::AVPixelFormat::AV_PIX_FMT_VAAPI
        {
            return Ok(());
        }
    }
    ...
    Err(anyhow!(
        "codec {} has no VAAPI hardware decoder; hardware decoding is mandatory",
        name.to_string_lossy()
    ))
}

/// Rejects everything but the VAAPI hardware pixel format: returning
/// NONE makes `avcodec` fail decoding instead of silently falling back
/// to software frames.
unsafe extern "C" fn require_vaapi_format(
