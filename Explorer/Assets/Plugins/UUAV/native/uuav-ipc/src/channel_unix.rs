//! macOS/Linux transport: one `AF_UNIX` `SOCK_STREAM` socketpair.
//!
...
//! reassembles partial reads across calls; Linux keeps the same framing
//! so both platforms share one codepath.
//!
//! This channel carries messages only. Shared-surface handles travel on
//! their own dedicated channel (`mach_channel` on macOS, `fd_channel`
//! on Linux), keeping this stream free of ancillary data.
//!
//! The fd stays blocking: sends block on a full kernel buffer
//! (backpressure replacing zmq's high-water mark), while reads use
//! `MSG_DONTWAIT` per call. A raw send to a dead helper must surface as
//! `EPIPE`, not deliver SIGPIPE to the Unity process: macOS sets
//! `SO_NOSIGPIPE` on both fds at creation, Linux passes `MSG_NOSIGNAL`
//! per send.
...
impl Channel {
    pub fn pair(_token: &str) -> anyhow::Result<(Self, ChildHandoff)> {
        ...
        let owned_parent = FdGuard(parent);
        let owned_child = FdGuard(child);
    ...
    fn write_all(&mut self, bytes: &[u8]) -> anyhow::Result<()> {
        ...
        while offset < bytes.len() {
            ...
            let written =
                unsafe { libc::send(self.fd, rest.as_ptr().cast(), rest.len(), SEND_FLAGS) };
            if written < 0 {
                let err = std::io::Error::last_os_error();
                match err.raw_os_error() {
