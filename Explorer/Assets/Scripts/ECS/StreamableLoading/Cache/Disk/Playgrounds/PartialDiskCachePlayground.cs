...
namespace ECS.StreamableLoading.Cache.Disk.Playgrounds
{
    public class PartialDiskCachePlayground : MonoBehaviour
    {
        ...
        private IPartialDiskCache NewDiskCache() =>
            new PartialDiskCache(CacheDirectory.New(cacheDirectory), new FilesLock(), IDiskCleanUp.None.INSTANCE);

        private async UniTaskVoid StartAsync()
        {
            string testExtension = "part";

            IPartialDiskCache diskCache = NewDiskCache();
            using HashKey hashKey = HashKey.FromString(TEST_FILE);
            var partialFile = await diskCache.PartialFileAsync(hashKey, testExtension, destroyCancellationToken);

            var originData = texture.GetRawTextureData<byte>();
            var data = new PartialLoadingState(partialFile.Unwrap());

            // data.AppendDataAsync(originData.mem());
            //
