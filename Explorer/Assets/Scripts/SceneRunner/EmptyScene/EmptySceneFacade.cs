...
namespace SceneRunner.EmptyScene
{
    public class EmptySceneFacade : ISceneFacade
    {
        ...
        internal Entity sceneRoot { get; private set; }
        internal Entity grass { get; private set; }
        internal Entity environment { get; private set; }
        ...
        public UniTask StartUpdateLoop(int targetFPS, CancellationToken ct)
        {
            ...
            SDKTransform grassTransform = transformPool.Get();
            grassTransform.Position = GLTF_POSITION;
            grassTransform.ParentId = sceneRoot.Id;
            PBGltfContainer grassGltf = gltfContainerPool.Get();
            grassGltf.Src = mapping.grass.file;
            grass = sharedWorld.Create(grassTransform, grassGltf, args.ParentPartition, partitionPool.Get());

            SDKTransform envTransform = transformPool.Get();
            envTransform.Position = GLTF_POSITION;
            envTransform.ParentId = sceneRoot.Id;
            PBGltfContainer envGltf = gltfContainerPool.Get();
            envGltf.Src = mapping.environment.file;
            environment = sharedWorld.Create(envTransform, envGltf, args.ParentPartition, partitionPool.Get());
