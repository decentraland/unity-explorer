...
{
    [ExecuteInEditMode]
    [AddComponentMenu("Ocean/Water Grid")]
    public class WaterGrid : MonoBehaviour
    {
        ...
        [SerializeField]
        [HideInInspector]
        private List<OceanObject> objects = new List<OceanObject>();

        [NonSerialized]
        private float tileSize;
        [NonSerialized]
        private OceanObject m_waterObject = null;
        [NonSerialized]
        ...
        public void Recreate()
        {
            ...
            {
                foreach (OceanObject obj in objects)
                {
            ...
            for (int x = 0; x < rowsColumns; x++)
            {
                for (int z = 0; z < rowsColumns; z++)
                {
                    if (requireRecreate)
                    {
                        m_waterObject = OceanObject.New(material, mesh);
        ...
        private void OnDrawGizmosSelected()
        {
            if (DisplayWireframe)
            {
                ...
                foreach (OceanObject waterObject in objects)
                {
                    if(waterObject.meshFilter.sharedMesh)
                        Gizmos.DrawWireMesh(waterObject.meshFilter.sharedMesh, waterObject.transform.localPosition);
                }
            }
