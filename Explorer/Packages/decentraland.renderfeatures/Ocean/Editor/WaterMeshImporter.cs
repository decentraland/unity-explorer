...
namespace DCL.Rendering.RenderGraphs.RenderFeatures.Ocean
{
    ...
    public class WaterMeshImporterEditor: ScriptedImporterEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (previewInSceneView && OceanObject.Instances.Count > 0)
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    GUILayout.Space(EditorGUIUtility.labelWidth);
                    EditorGUILayout.HelpBox($"Drawing on WaterObject instances in the scene ({OceanObject.Instances.Count})", MessageType.None);
                }
            }
        ...
        private void OnSceneGUI(SceneView obj)
        {
            ...
            {
                GL.wireframe = true;
                if (OceanObject.Instances.Count > 0)
                {
                    foreach (OceanObject waterObject in OceanObject.Instances)
                    {
