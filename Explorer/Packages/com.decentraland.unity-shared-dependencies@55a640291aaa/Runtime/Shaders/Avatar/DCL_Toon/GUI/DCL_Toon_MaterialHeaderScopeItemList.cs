...
namespace UnityEditor.Rendering.DCL_Toon
{
    ...
    internal class DCL_Toon_MaterialHeaderScopeList
    {
        ...
        internal readonly List<DCL_Toon_MaterialHeaderScopeItem> m_Items = new ();
        ...
        /// ///
        internal void RegisterHeaderScope<TEnum>(GUIContent title, TEnum expandable, Action<Material> action, uint workflowMode, uint isTransparent,
            uint isTessellation)
            where TEnum: struct, IConvertible
        {
            m_Items.Add(new DCL_Toon_MaterialHeaderScopeItem
            {
                ...
                url = DCL_Toon_DocumentationUtils.GetHelpURL(expandable),
        ...
        internal void DrawHeaders(MaterialEditor materialEditor, Material material)
        {
            ...
            foreach (DCL_Toon_MaterialHeaderScopeItem item in m_Items)
            {
