...
{
    internal class DCL_Toon_GUI : ShaderGUI
    {
        ...
        internal static RenderPipeline currentRenderPipeline
        {
            get
            {
                ...
                if (currentRenderPipeline == null) { return RenderPipeline.Legacy; }

                if (currentRenderPipeline.defaultMaterial.shader.name.StartsWith(kHdrpShaderPrefix)) { return RenderPipeline.HDRP; }

                if (currentRenderPipeline.defaultMaterial.shader.name.StartsWith(kUrpShaderPrefix)) { return RenderPipeline.Universal; }
        ...
        [Flags]
        [DCL_Toon_InspectorHelpURL("instruction")]
        ...
        internal int m_renderQueue = (int)RenderQueue.Geometry;
        ...
        private UTS_Mode m_WorkflowMode;

        private readonly DCL_Toon_MaterialHeaderScopeList m_MaterialScopeList = new (uint.MaxValue & ~(uint)Expandable.Basic);
        ...
        protected bool _ShadowControlMaps_Foldout;
        protected bool _AdditionalLookdevs_Foldout;
        ...
        protected MaterialProperty specularMode;
        protected MaterialProperty specularBlendMode;
        protected MaterialProperty matcapCameraMode;
        protected MaterialProperty matcapBlendMode;
        protected MaterialProperty matcapOrtho = null;
        protected MaterialProperty transparentMode;
        protected MaterialProperty clippingMode;
        protected MaterialProperty clippingMask;
        protected MaterialProperty stencilMode;
        protected MaterialProperty mainTex;
        protected MaterialProperty baseColor;
        protected MaterialProperty firstShadeMap;
        protected MaterialProperty firstShadeColor;
        protected MaterialProperty secondShadeMap;
        protected MaterialProperty secondShadeColor;
        protected MaterialProperty normalMap;
        protected MaterialProperty bumpScale;
        protected MaterialProperty set_1st_ShadePosition;
        protected MaterialProperty set_2nd_ShadePosition;
        protected MaterialProperty highColor_Tex;
        protected MaterialProperty highColor;
        protected MaterialProperty set_HighColorMask;
        protected MaterialProperty rimLight_Power = null;
        protected MaterialProperty ap_RimLight_Power = null;
        protected MaterialProperty set_RimLightMask;
        protected MaterialProperty matCap_Sampler;
        protected MaterialProperty matCapColor;
        protected MaterialProperty normalMapForMatCap;
        protected MaterialProperty bumpScaleMatcap;
        protected MaterialProperty set_MatcapMask;
        protected MaterialProperty emissive_Tex;
        protected MaterialProperty emissive_Color;
        protected MaterialProperty rotate_EmissiveUV = null;
        protected MaterialProperty outline_Sampler;
        protected MaterialProperty offset_Z = null;
        protected MaterialProperty outlineTex;
        protected MaterialProperty bakedNormal;
        ...
        private static bool FoldoutSubMenu(bool display, GUIContent title) =>
            EditorGUILayout.Foldout(display, title, true);

        private class RangeProperty
        {
            internal readonly GUIContent m_GuiContent;
            ...
            internal RangeProperty(string label, string tooltip, string propName, float defaultValue, float min,
                float max)
            {
        ...
        private class ColorProperty
        {
            internal readonly GUIContent m_GuiContent;
            internal readonly string m_propertyName;
            internal readonly bool m_isHDR;
        ...
        private static class Styles
        {
            ...
            public static readonly FloatProperty farthestDistanceText = new (label: "Farthest Distance to vanish",
            ...
            public static readonly FloatProperty nearestDistanceText = new (label: "Nearest Distance to draw with Outline Width",
        ...
        private void OnOpenGUI(Material material, MaterialEditor materialEditor, MaterialProperty[] props)
        {
        ...
        private void UTS3DrawHeaders(MaterialEditor materialEditor, Material material)
        {
            if (material == null)
            ...
            foreach (DCL_Toon_MaterialHeaderScopeItem item in m_MaterialScopeList.m_Items)
            {
                if (transparencyEnabled == UTS_TransparentMode.On && item.transparentEnabled == (uint)UTS_TransparentMode.On) { continue; }
        ...
        private void ShaderPropertiesGUI(MaterialEditor materialEditor, Material material, MaterialProperty[] properties)
        {
            UTS3DrawHeaders(materialEditor, material);
        }

        private void DrawShaderOptions(Material material)
        {
            ...
            switch (m_WorkflowMode)
            {
                case UTS_Mode.ThreeColorToon:
                    ...
                    var mode0 = (UTS_ClippingMode)MaterialGetInt(material, ShaderPropClippingMode);
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
        {
            ...
            if (transparencyEnabled != UTS_TransparentMode.On) { SetupOutline(material); }
            else { SetupOverDrawTransparentObject(material); }
        ...
        private bool GUI_ToggleShaderKeyword(Material material, string label, string keyword)
        {
            bool isEnabled = material.IsKeywordEnabled(keyword);

            EditorGUI.BeginChangeCheck();
            bool ret = EditorGUILayout.Toggle(label, isEnabled);

            if (EditorGUI.EndChangeCheck())
            {
                m_MaterialEditor.RegisterPropertyChangeUndo(label);

                if (ret == false) { material.DisableKeyword(keyword); }
        ...
        private float GUI_FloatProperty(Material material, FloatProperty floatProp)
        {
        ...
        private Color GUI_ColorProperty(Material material, ColorProperty colorProp)
        {
        ...
        private float GUI_RangeProperty(Material material, RangeProperty rangeProp) =>
            GUI_RangeProperty(material, rangeProp.m_GuiContent, rangeProp.m_propertyName, rangeProp.m_Min, rangeProp.m_Max);

        private float GUI_RangeProperty(Material material, GUIContent guiContent, string propName, float min, float max)
        {
        ...
        private bool GUI_Toggle(Material material, GUIContent guiContent, string prop, bool value)
        {
            EditorGUI.BeginChangeCheck();
        ...
        private void GUI_SetCullingMode(Material material)
        {
            ...
            if ((int)CullingMode.Off == _CullMode_Setting) { m_cullingMode = CullingMode.Off; }
            else if ((int)CullingMode.Frontface == _CullMode_Setting) { m_cullingMode = CullingMode.Frontface; }
            else { m_cullingMode = CullingMode.Backface; }
        ...
        private void GUI_Tranparent(Material material)
        {
            ...
            DoPopup(Styles.transparentModeText, transparentMode, Enum.GetNames(typeof(UTS_TransparentMode)));
            transparencyEnabled = (UTS_TransparentMode)MaterialGetInt(material, ShaderPropTransparentEnabled);
        ...
        private void GUI_BasicThreeColors(Material material)
        {
            ...
            bool applyTo1st = GUI_Toggle(material, Styles.applyTo1stShademapText, ShaderPropUse_BaseAs1st, MaterialGetInt(material, ShaderPropUse_BaseAs1st) != 0);
            EditorGUI.indentLevel -= 2;
            ...
            else { m_MaterialEditor.TexturePropertySingleLine(Styles.firstShadeColorText, firstShadeMap, firstShadeColor); }

            EditorGUI.indentLevel += 2;
            bool applyTo2nd = GUI_Toggle(material, Styles.applyTo2ndShademapText, ShaderPropUse_1stAs2nd, MaterialGetInt(material, ShaderPropUse_1stAs2nd) != 0);
            EditorGUI.indentLevel -= 2;
            ...
            else { m_MaterialEditor.TexturePropertySingleLine(Styles.secondShadeColorText, secondShadeMap, secondShadeColor); }
        ...
        private void GUI_ShadingStepAndFeatherSettings(Material material)
        {
            EditorGUILayout.LabelField("Mode: Standard", EditorStyles.boldLabel);
        ...
        private void GUI_NormalmapSettings(Material material)
        {
        ...
        private void GUI_HighlightSettings(Material material)
        {
            ...
            if (!_SimpleUI)
            {
                ...
                EditorGUI.BeginChangeCheck();
                mode = EditorGUILayout.Popup(Styles.specularModeText, mode, Enum.GetNames(typeof(UTS_SpeculerMode)));
                ...
                var blendingMode = (int)specularBlendMode.floatValue;
                EditorGUI.BeginChangeCheck();
                blendingMode = EditorGUILayout.Popup(Styles.specularBlendModeText, blendingMode, Enum.GetNames(typeof(UTS_SpeculerColorBlendMode)));
                ...
                bool ret = GUI_Toggle(material, Styles.highlightOnShadowText, ShaderPropIs_UseTweakHighColorOnShadow, MaterialGetInt(material, ShaderPropIs_UseTweakHighColorOnShadow) != 0);
        ...
        {
            EditorGUILayout.BeginHorizontal();
            bool rimLightEnabled = GUI_Toggle(material, Styles.rimlightText, ShaderPropRimLight, MaterialGetInt(material, ShaderPropRimLight) != 0);
            EditorGUILayout.EndHorizontal();
            ...
            if (!_SimpleUI)
            {
                ...
                bool direcitonMaskEnabled = GUI_Toggle(material, Styles.rimlightDirectionMaskText, ShaderPropLightDirection_MaskOn, MaterialGetInt(material, ShaderPropLightDirection_MaskOn) != 0);
                EditorGUILayout.EndHorizontal();
...

                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    bool antipodean_RimLight = GUI_Toggle(material, Styles.invertedRimlightText, ShaderPropAdd_Antipodean_RimLight, MaterialGetInt(material, ShaderPropAdd_Antipodean_RimLight) != 0);
                    EditorGUILayout.EndHorizontal();
        ...
        {
            EditorGUILayout.BeginHorizontal();
            bool matcapEnabled = GUI_Toggle(material, Styles.matCapText, ShaderPropMatCap, MaterialGetInt(material, ShaderPropMatCap) != 0);
            EditorGUILayout.EndHorizontal();
            ...
            if (!_SimpleUI)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                DoPopup(Styles.matcapBlendModeText, matcapBlendMode, Enum.GetNames(typeof(UTS_MatcapColorBlendMode)));
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal();
                bool isNormalMapForMatCap = GUI_Toggle(material, Styles.matCapNormalmapSpecularaMask, ShaderPropIs_NormalMapForMatCap, MaterialGetInt(material, ShaderPropIs_NormalMapForMatCap) != 0);
                ...
                EditorGUILayout.BeginHorizontal();
                bool tweakMatCapOnShadows = GUI_Toggle(material, Styles.matCapOnShadow, ShaderPropIs_UseTweakMatCapOnShadow, MaterialGetInt(material, ShaderPropIs_UseTweakMatCapOnShadow) != 0);
                ...
                EditorGUI.EndDisabledGroup();
                DoPopup(Styles.matcapOrthoText, matcapCameraMode, Enum.GetNames(typeof(CameraProjectionType)));
            }
        ...
        private void ApplyQueueAndRenderType(UTS_Mode technique, Material material)
        {
            ...
            string ignoreProjection = DONT_IGNOREPROJECTION;
            ...
            else
            {
                switch (technique)
                {
                    case UTS_Mode.ThreeColorToon:
                    {
                        ...
                        else { renderType = TRANSPARENTCUTOUT; }

                        break;
                    }
                    case UTS_Mode.ShadingGradeMap:
                    {
                        var transClippingMode = (UTS_TransClippingMode)MaterialGetInt(material, ShaderPropClippingMode);
        ...
        {
            var mode = (UTS_StencilMode)MaterialGetInt(material, ShaderPropStencilMode);
        ...
        private void GUI_Emissive(Material material)
        {
            ...
            bool ret = EditorGUILayout.Toggle(Styles.emissionAnimationText, m_emissionMode != EmissionMode.SimpleEmissive);
        ...
        private void GUI_Outline(Material material)
        {
            ...
            if (srpDefaultLightModeTag == srpDefaultLightModeName)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (isOutlineEnabled)
                    {
                        if (isLegacy) { material.DisableKeyword(kDisableOutlineKeyword); }

                        material.SetShaderPassEnabled(srpDefaultLightModeName, true);
                    }
                    else
                    {
                        if (isLegacy) { material.EnableKeyword(kDisableOutlineKeyword); }
            ...
            if ((int)OutlineMode.NormalDirection == _OutlineMode_Setting) { m_outlineMode = OutlineMode.NormalDirection; }
            else if ((int)OutlineMode.PositionScaling == _OutlineMode_Setting) { m_outlineMode = OutlineMode.PositionScaling; }
            ...
            if (!_SimpleUI)
            {
                ...
                bool useOutlineTexture = GUI_Toggle(material, Styles.outlineColorMapText, ShaderPropIs_OutlineTex, MaterialGetInt(material, ShaderPropIs_OutlineTex) != 0);
                ;
                ...
                {
                    bool isBackedNormal = GUI_Toggle(material, Styles.bakedNormalForOutlineText, ShaderPropIs_BakedNormal, MaterialGetInt(material, ShaderPropIs_BakedNormal) != 0);
        ...
        private void GUI_LightColorEffectiveness(Material material)
        {
            ...
            EditorGUI.BeginChangeCheck();
            string prop = ShaderPropIs_Filter_LightColor;

            int value = MaterialGetInt(material, prop);
            bool ret = EditorGUILayout.Toggle(Styles.limitLightIntensityText.text, value != 0);

            if (EditorGUI.EndChangeCheck())
            {
                int boolValue = ret ? 1 : 0;
        ...
        private void GUI_MetaverseSettings(Material material)
        {
...

            {
                ...
                bool isBold = GUI_Toggle(material, Styles.metaverseLightDirectionText, ShaderPropIs_BLD, MaterialGetInt(material, ShaderPropIs_BLD) != 0);
