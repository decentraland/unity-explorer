...
namespace UnityEditor.Rendering
{
    ...
    {
        private const string k_KeyPrefix = "CoreRP:Material:UI_State:";
        ...
        internal static bool IsAreaExpanded(this MaterialEditor editor, uint mask, uint defaultExpandedState = uint.MaxValue)
        {
            ...
            {
                var state = (uint)EditorPrefs.GetInt(key);
                return (state & mask) > 0;
            }
        ...
        private static string GetEditorPrefsKey(this MaterialEditor editor) =>
            k_KeyPrefix + (editor.target as Material).shader.name;
    }
    ...
    public static partial class MaterialEditorExtension
    {
        ...
        {
            Vector2 limits = prop.rangeLimits;
        ...
        public static void Vector3ShaderProperty(this MaterialEditor editor, MaterialProperty prop, GUIContent label)
        {
            ...
            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = prop.hasMixedValue;
            Vector3 vector = EditorGUILayout.Vector3Field(label, prop.vectorValue);
            EditorGUI.showMixedValue = false;
        ...
        /// <param name="label">Label for the property</param>
        public static void MinMaxShaderProperty(this MaterialEditor editor, MaterialProperty min, MaterialProperty max, float minLimit, float maxLimit,
            GUIContent label)
        {
