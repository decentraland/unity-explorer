...
namespace LiveKit.Internal
{
    ...
    internal sealed class FfiClient : IFFIClient
    {
        ...
        static void Init()
        {
            // The avatar-render-server build never connects to comms; the native FFI
            // (LiveKitInitialize) segfaults during boot in a headless standalone player.
            if (System.Array.IndexOf(System.Environment.GetCommandLineArgs(), "--avatar-render-server") >= 0
                || System.Environment.GetEnvironmentVariable("DCL_AVATAR_RENDER_SERVER") == "1")
                return;
